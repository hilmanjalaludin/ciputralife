...
class cmp_tracking_overview2 extends index
{
	...
	private function getCampaignInfo()
	{
		...
		if($this->havepost('Campaign'))
		{
			$sql = "select
					...
					#AND cs.CustomerUpdatedTs <= '".$this->end_date." 23:59:59'
					";
			...
			foreach($qry -> result_assoc() as $rows )
			{
				$data[$rows['UserId']]['spv'] = $rows['spv'];
				$data[$rows['UserId']]['agentId'] = $rows['agentId'];
				$data[$rows['UserId']]['agent_name'] = $rows['agent_name'];
				$data[$rows['UserId']]['datasize'] = $rows['datasize'];
				$data[$rows['UserId']]['UserId'] = $rows['UserId'];
	...
	private function getReasonCall()
	{
		...
		if($this->havepost('Campaign'))
		{
			...
			foreach($qry -> result_assoc() as $rows )
			{
				$data[$rows['UserId']]['BUSY'] += (INT)$rows['BUSY'];	
				$data[$rows['UserId']]['INVALID_NUMBER'] += (INT)$rows['INVALID_NUMBER'];		
				$data[$rows['UserId']]['NO_PICK'] += (INT)$rows['NO_PICK'];		
				$data[$rows['UserId']]['CALL_AGAIN'] += (INT)$rows['CALL_AGAIN'];		
				$data[$rows['UserId']]['MISS_CUSTOMER'] += (INT)$rows['MISS_CUSTOMER'];		
				$data[$rows['UserId']]['THINKING'] += (INT)$rows['THINKING'];		
				$data[$rows['UserId']]['MOVED'] += (INT)$rows['MOVED'];		
				$data[$rows['UserId']]['OVERAGE'] += (INT)$rows['OVERAGE'];		
				$data[$rows['UserId']]['DONOTCALL'] += (INT)$rows['DONOTCALL'];		
				$data[$rows['UserId']]['NO_CARD'] += (INT)$rows['NO_CARD'];		
				$data[$rows['UserId']]['NOT_INTEREST'] += (INT)$rows['NOT_INTEREST'];		
				$data[$rows['UserId']]['WRONG_PERSON'] += (INT)$rows['WRONG_PERSON'];		
				$data[$rows['UserId']]['FOLLOWUP_EMAIL'] += (INT)$rows['FOLLOWUP_EMAIL'];		
				$data[$rows['UserId']]['FOLLOWUP_WA'] += (INT)$rows['FOLLOWUP_WA'];
				$data[$rows['UserId']]['SALES'] += (INT)$rows['SALES'];		
				$data[$rows['UserId']]['Thinking_WA'] += (INT)$rows['Thinking_WA'];
				$data[$rows['UserId']]['Thinking_Email'] += (INT)$rows['Thinking_Email'];
				$data[$rows['UserId']]['Agree_WA'] += (INT)$rows['Agree_WA'];
				$data[$rows['UserId']]['Agree_Email'] += (INT)$rows['Agree_Email'];
				$data[$rows['UserId']]['Reject_Email'] += (INT)$rows['Reject_Email'];
				$data[$rows['UserId']]['Reject_WA'] += (INT)$rows['Reject_WA'];
				$data[$rows['UserId']]['Touch'] += (INT)$rows['Touch'];
			}
		}
	...
	private function getCasesAPE()
	{
		...
		if($this->havepost('Campaign'))
		{
			...
			foreach($qry -> result_assoc() as $rows )
			{
				...
				$data[$rows['UserId']]['APE'] += (int)$rows['APE'];
				$data[$rows['UserId']]['PREMI'] += (int)$rows['PREMI'];
				$data[$rows['UserId']]['CASES'] += (int)$rows['CASES'];
			}
		}
	...
	private function getAttempt()
	{
		...
		if($this->havepost('Campaign'))
		{
			$sql = "SELECT DISTINCT b.CustomerId,
					...
					and b.FuId IS NULL and b.TypeFa IS NULL";
			...
			foreach($qry -> result_assoc() as $rows )
			{
				...
				$data[$rows['UserId']]['CallAttempt'] = $rows['CallAttempt'];				
	...
	private function getReason()
	{
		...
		if($this->havepost('Campaign'))
		{
			...
			foreach($qry -> result_assoc() as $rows )
			{
				...
				$data[$rows['UserId']]['contacted'] += (int)$rows['contacted'];				
				$data[$rows['UserId']]['uncontacted'] += (int)$rows['uncontacted'];
	...
	private function write_header_closed()
	{
		echo "<table class=\"grid\" cellpadding=\"0\" cellspacing=\"0\" >
				<tr>
					...
					<td rowspan=\"2\" class=\"header first\" nowrap align=\"center\">SPV Name</td>
					<td rowspan=\"2\" class=\"header middle\" nowrap align=\"center\">Agent ID</td>
					<td rowspan=\"2\" class=\"header middle\" nowrap align=\"center\">Agent Name</td>
	...
	private function write_content()
	{
		...
		foreach($CampaignInfo as $up_id => $arr_val)
		{
			...
			echo "<tr>
					...
					<td rowspan=\" ".$rowspan." \" class=\"content middle\" nowrap>".$arr_val['spv']."</td>
					<td rowspan=\" ".$rowspan." \" class=\"content middle\" nowrap>".$arr_val['agentId']."</td>
					<td rowspan=\" ".$rowspan." \" class=\"content middle\" nowrap>".$arr_val['agent_name']."</td>
		...
		echo "	<tr>
				<td colspan=\"4\" class=\"header middle\" nowrap align=\"center\">Total</td>
	...
	{
		echo "	<tr>
					<th colspan=\"4\" class=\"header middle\" nowrap align=\"center\">subtotal</th>
