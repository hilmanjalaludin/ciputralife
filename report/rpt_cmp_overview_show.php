	...
	set_time_limit(500000);
		...
		function anp()
		{
			...
			$sql = "SELECT
						...
						SUM(b.Premi) AS tmp, SUM(IF(e.PayModeId=2,c.,(b.Premi*12), b.Premi)) AS ANP
					FROM t_gn_policyautogen a
						LEFT JOIN t_gn_policy b ON a.PolicyNumber=b.PolicyNumber
						LEFT JOIN t_gn_customer c ON a.CustomerId=c.CustomerId
						LEFT JOIN t_gn_assignment d ON c.CustomerId=d.CustomerId
		...
		function anpgross()
		{
			...
			$sql = "SELECT
						f.CampaignId, f.CampaignNumber,
						SUM(b.Premi) AS tmp, SUM(IF(e.PayModeId=2,c.,(b.Premi*12), b.Premi)) AS anpgross
					FROM t_gn_policyautogen a
						LEFT JOIN t_gn_policy b ON a.PolicyNumber=b.PolicyNumber
						LEFT JOIN t_gn_customer c ON a.CustomerId=c.CustomerId
						LEFT JOIN t_gn_assignment d ON c.CustomerId=d.CustomerId
					...
					WHERE
						DATE(b.PolicySalesDate)>='".$_REQUEST['start_date']."' AND
						DATE(b.PolicySalesDate)<='".$_REQUEST['end_date']."'
						AND c.CallReasonId IN(15)
					GROUP BY f.CampaignNumber ";
			// echo "<pre>".$sql."</pre>";
			$qry=mysql_query($sql);
			while ($row = mysql_fetch_array($qry))
			{
				$anpgross[$row['CampaignNumber']] = $row['anpgross'];
			}
			return $anpgross;
		}		
		...
		function getCases($CampaignId,$UploadId,$cat=""){
			...
			switch($cat){
				case "cases":
					// $sql.=" AND tcll.CallReasonCategoryId IN (4,5) AND tgpd.ProductId = 8";
					$sql.=" AND tcst.CallReasonId = 15 AND tcst.CallReasonQue = 1 AND DATE(tcst.CustomerUpdatedTs) >= '".$_REQUEST['start_date']."'
					AND DATE(tcst.CustomerUpdatedTs) <= '".$_REQUEST['end_date']."';";
					break;
				case "contacted":
					// $sql.=" AND tcll.CallReasonCategoryId IN (2,3,4)";
					$sql.=" AND tcll.CallReasonContactedFlag = 1 AND DATE(tcst.CustomerUpdatedTs) >= '".$_REQUEST['start_date']."'
					AND DATE(tcst.CustomerUpdatedTs) <= '".$_REQUEST['end_date']."';";
					break;
				case "nottouch":
					$sql.=" AND tcst.CallReasonId IS NULL;";
					break;
				case "touch":
					$sql.=" AND tcst.CallReasonId IS NOT NULL AND DATE(tcst.CustomerUpdatedTs) >= '".$_REQUEST['start_date']."'
					AND DATE(tcst.CustomerUpdatedTs) <= '".$_REQUEST['end_date']."';";
					break;
				case "gross":
					$sql.=" AND tcst.CallReasonId = 15 AND DATE(tcst.CustomerUpdatedTs) >= '".$_REQUEST['start_date']."'
					AND DATE(tcst.CustomerUpdatedTs) <= '".$_REQUEST['end_date']."';";
					break;
			}
			// echo "<pre>".$sql."</pre>";
...
<thead>
	...
	<tr height="30">
		...
		<!--<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;">&nbsp;Cases (Gross)</th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;">&nbsp;APE (Gross)</th>-->
...
<tbody>
	<?php
		...
		while($row = $db ->fetchrow($ListPages->result))
		{
			...
			$oANPgross		= ($gross[$row->CampaignNumber] ? $gross[$row->CampaignNumber] :0);
			//new line
			$cases   	= getCases($row->CampaignId,$row->UploadId,"cases");
			$contact 	= getCases($row->CampaignId,$row->UploadId,"contacted");
			$casesgross	= getCases($row->CampaignId,$row->UploadId,"gross");
	...
	?>
			...
			<tr>
				...
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format($tot_data,0,",",".") ; ?></td>
				<?php
					// $prodCode = getProductCode(1);
					// foreach($prodCode as $key => $val){
					...
					// echo "</pre>";
				?>
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format($oCases,0,",",".") ; ?></td>
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format($oANP,0,",",".") ; ?></td>
				<!--remarks->>
				<!--<td nowrap style="text-align: center" class="content-middle"><?php //echo number_format($ocasesgross,0,",",".") ; ?></td>
				<td nowrap style="text-align: center" class="content-middle"><?php //echo number_format($oANPgross,0,",",".") ; ?></td>-->
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format($oContact,0,",",".") ; ?></td>
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format($oAttempt,0,",",".") ; ?></td>
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format($oNotTouch,0,",",".") ; ?></td>
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format($oTouch,0,",",".") ; ?></td>
				<!--AVG /APE -->
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format($oANP/$oCases);?></td>
				<!-- CONTACT RATE -->
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format(ROUND(($oContact/$oTouch)*100,2),2,",","."). "%"; ?></td>
				<!-- RESPONSE RATE -->
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format(ROUND(($oCases/$oTouch)*100,5),5,",","."). "%"; ?></td>
				<!-- CONVERSION RATE -->
				<!-- UPDATE RUMUS 11-10-2017 REQ BY BU MINI -->
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format(ROUND(($oCases/$oContact)*100,2),2,",","."). "%"; ?></td>
				...
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format(ROUND(($oAttempt/$oTouch),2),2,",","."); ?></td>
			</tr>
	<?php
		/** KALKULASI **/
			$no++;
	...
	<tr height="30">
		...
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format($sTotalData,0,",","."); ?></th>
		<?php
					// $prodCode = getProductCode(1);
						// foreach($prodCode as $key => $val){
		...
		?>		
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format($sCases,0,",",".") ; ?></th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format($sANP,0,",",".") ; ?></th>
		<!--REMARK NJIR
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php //echo number_format($sCasesgross,0,",",".") ; ?></th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php //echo number_format($sANPgross,0,",",".") ; ?></th>-->
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format($sContact,0,",",".") ;?></th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format($sAttempt,0,",",".") ; ?></th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format($sNotTouch,0,",",".") ;  ?></th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format($sTouch,0,",",".") ; ?></th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format($tAVGNBI,0,",",".") ; ?></th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format(ROUND(($tContactRate)*100,2),2,",",".")."%"; ?></th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format(ROUND(($tResponseRate)*100,5),5,",",".")."%"; ?></th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format(ROUND(($tConversiRate)*100,2),2,",",".")."%"; ?></th>
		<th nowrap bgcolor="#3366FF" class="custom-grid th-middle" style="color:#FFFFFF;text-align:center;"><?php echo number_format(ROUND(($tAttemptRatio),2),2,",","."); ?></th>
	</tr>
	</div>
	</tbody>
</table>
