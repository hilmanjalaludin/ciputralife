...
?> 
	...
	<script>
		function ShowReport($CignaSystemCode){
			...
			var camptype = $('#status').val();
			var status = $('#status').val();
				if (start_date !=0 && end_date != 0){
					if( confirm('Do you want to show this Report..?')){
						var url  = "../report/rpt_calltracking_show.php?action=ShowReport&start_date="+start_date+"&end_date="+end_date+"&camptype="+camptype;
					window.open(url);
					}else 
						return false;
				}else
		...
		function getIframe(){
			var iframe_window_txt = doJava.dom('iframe_window_txt');
				...
				iframe_window_txt.src='../DownLoadReport/calltracking/'
		}
			...
			var status = $('#status').val();
				if (start_date !=0){
					if( confirm('Do you want to Download this Report..?')){
						...
						doJava.File = "../report/rpt_calltracking_download.php";
						doJava.Params = {
							action		:'ShowReport',
	...
	<fieldset class="corner">
		<legend class="icon-product"> &nbsp;&nbsp;&nbsp;Form Report Download Call Tracking</legend>
					...
					<div class="sub_main_content">
						<table cellpadding="5px" align="left">
							...
							<tr>
								<th>Campaign Type</th>
								<td> 
									<select style="width:200px" name="status" id="status">
							...
							<tr>
								<th> * Cigna System</th>
								<td>
									<select name="CignaSystem" id="CignaSystem">
											<option value=""> -- Choose -- </option>
			...
			<fieldset style="border:1px solid #dddddd;">
				<legend style="font-size:14px;"><font color=red>Download CallTracking txt Files </legend>
