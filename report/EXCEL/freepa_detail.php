...
class freepa_detail extends index
{
	...
	private function getCampaignInfo()
	{
		...
		if($this->escPost('Option') == 1)
		{
			...
			$sql = "select fp.CustomerId as InsuredId,
				concat(i.InsuredFirstName,' ',i.InsuredLastName) as customername,
				date_format(i.InsuredDOB, '%d-%m-%Y') as DOB,
				p.PayerEmail as  Email,
				p.PayerMobilePhoneNum as HP,
			...
			left join t_gn_insured i on i.CustomerId=fp.CustomerId
			left join t_gn_payer p on p.CustomerId=fp.CustomerId
			left join t_lk_province pr on pr.ProvinceId=p.ProvinceId
			inner join t_gn_multians_survey ans on ans.customer_id=fp.CustomerId
			left join tms_agent a on i.CreatedById=a.UserId
			left join tms_agent b on a.spv_id=b.UserId
			left join tms_agent e on a.manager_id=e.UserId #manager
			left join tms_agent c on a.mgr_id=c.UserId #am
			where ans.quest_have_ans=1
				#and f.ProductId=2
		...
		if($this->escPost('Option') == 1) {
			...
			foreach($qry -> result_assoc() as $rows )
			{
				...
				$data[$rows['InsuredId']]['customername'] = $rows['customername'];
				$data[$rows['InsuredId']]['DOB'] = $rows['DOB'];
				$data[$rows['InsuredId']]['Email'] = $rows['Email'];
				$data[$rows['InsuredId']]['HP'] = $rows['HP'];
				$data[$rows['InsuredId']]['Alamat'] = $rows['Alamat'];
				$data[$rows['InsuredId']]['provinsi'] = $rows['provinsi'];
				$data[$rows['InsuredId']]['Survey'] = $rows['Survey'];
				$data[$rows['InsuredId']]['agentcode'] = $rows['agentcode'];
				$data[$rows['InsuredId']]['agentname'] = $rows['agentname'];
				$data[$rows['InsuredId']]['spvcode'] = $rows['spvcode'];
				$data[$rows['InsuredId']]['spvname'] = $rows['spvname'];
				$data[$rows['InsuredId']]['amcode'] = $rows['amcode'];
				$data[$rows['InsuredId']]['amname'] = $rows['amname'];
				$data[$rows['InsuredId']]['mgrcode'] = $rows['mgrcode'];
				$data[$rows['InsuredId']]['mgrname'] = $rows['mgrname'];
			}
		return $data;
	}
