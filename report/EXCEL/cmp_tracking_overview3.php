...
class cmp_tracking_overview3 extends index
{
	private $product_category;
	function cmp_tracking_overview3()
	{
	...
	private function getCampaignInfo()
	{
		...
		if($this->havepost('Campaign'))
		{
			...
			foreach($qry -> result_assoc() as $rows )
			{
				$data[$rows['spvid']]['spvid'] = $rows['spvid'];
				$data[$rows['spvid']]['spv'] = $rows['spv'];
				$data[$rows['spvid']]['spvname'] = $rows['spvname'];
				$data[$rows['spvid']]['datasize'] = $rows['datasize'];
			}
		}
	...
	private function getReasonCall()
	{
		...
		if($this->havepost('Campaign'))
		{
			...
			foreach($qry -> result_assoc() as $rows )
			{
				$data[$rows['spvid']]['BUSY'] += (INT)$rows['BUSY'];
				$data[$rows['spvid']]['INVALID_NUMBER'] += (INT)$rows['INVALID_NUMBER'];
				$data[$rows['spvid']]['NO_PICK'] += (INT)$rows['NO_PICK'];
				$data[$rows['spvid']]['CALL_AGAIN'] += (INT)$rows['CALL_AGAIN'];
				$data[$rows['spvid']]['MISS_CUSTOMER'] += (INT)$rows['MISS_CUSTOMER'];
				$data[$rows['spvid']]['THINKING'] += (INT)$rows['THINKING'];
				$data[$rows['spvid']]['MOVED'] += (INT)$rows['MOVED'];
				$data[$rows['spvid']]['OVERAGE'] += (INT)$rows['OVERAGE'];
				$data[$rows['spvid']]['DONOTCALL'] += (INT)$rows['DONOTCALL'];
				$data[$rows['spvid']]['NO_CARD'] += (INT)$rows['NO_CARD'];
				$data[$rows['spvid']]['NOT_INTEREST'] += (INT)$rows['NOT_INTEREST'];
				$data[$rows['spvid']]['WRONG_PERSON'] += (INT)$rows['WRONG_PERSON'];
				$data[$rows['spvid']]['FOLLOWUP_EMAIL'] += (INT)$rows['FOLLOWUP_EMAIL'];
				$data[$rows['spvid']]['FOLLOWUP_WA'] += (INT)$rows['FOLLOWUP_WA'];
				$data[$rows['spvid']]['SALES'] += (INT)$rows['SALES'];
				$data[$rows['spvid']]['Thinking_WA'] += (INT)$rows['Thinking_WA'];
				$data[$rows['spvid']]['Thinking_Email'] += (INT)$rows['Thinking_Email'];
				$data[$rows['spvid']]['Agree_WA'] += (INT)$rows['Agree_WA'];
				$data[$rows['spvid']]['Agree_Email'] += (INT)$rows['Agree_Email'];
				$data[$rows['spvid']]['Reject_Email'] += (INT)$rows['Reject_Email'];
				$data[$rows['spvid']]['Reject_WA'] += (INT)$rows['Reject_WA'];
				$data[$rows['spvid']]['Touch'] += (INT)$rows['Touch'];
			}
		}
	...
	private function getCasesAPE()
	{
		...
		if($this->havepost('Campaign'))
		{
			...
			foreach($qry -> result_assoc() as $rows )
			{
				...
				$data[$rows['spvid']]['APE'] += (int)$rows['APE'];
				$data[$rows['spvid']]['PREMI'] += (int)$rows['PREMI'];
				$data[$rows['spvid']]['CASES'] += (int)$rows['CASES'];
			}
		}
	...
	private function getAttempt()
	{
		...
		if($this->havepost('Campaign'))
		{
			...
			foreach($qry -> result_assoc() as $rows )
			{
				...
				$data[$rows['spvid']]['CallAttempt'] = $rows['CallAttempt'];
	...
	private function getReason()
	{
		...
		if($this->havepost('Campaign'))
		{
			...
			foreach($qry -> result_assoc() as $rows )
			{
				...
				$data[$rows['spvid']]['contacted'] += (int)$rows['contacted'];
				$data[$rows['spvid']]['uncontacted'] += (int)$rows['uncontacted'];
	...
	private function write_header_closed()
	{
		echo "<table class=\"grid\" cellpadding=\"0\" cellspacing=\"0\" >
				<tr>
					...
					<td rowspan=\"2\" class=\"header first\" nowrap align=\"center\">SPV ID</td>
					<td rowspan=\"2\" class=\"header middle\" nowrap align=\"center\">SPV Name</td>
