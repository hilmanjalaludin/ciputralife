...
// return $sql;
	...
	function getRowFileInsured($textFile='',$datas=array(),$page=0,$perpage=0){
		...
		$insured = "
		SELECT DISTINCT 
				CONCAT(date_format(cst.CustomerUpdatedTs, '%d/%m/%Y'),'-',pa.PolicyNumber) AS FormNumber,
				
				s.Salutation as title,
				ins.InsuredFirstName as InsuredName,
				DATE(ins.InsuredDOB) as InsuredDOB,
				ins.GenderId as sex,
				...
				'' as Children,
				'' as FACode,
				DATE(cst.CustomerUpdatedTs) as Activation_Date,
				'' as BranchCode,
				'' as CSName,
				...
				am.id as am_id,
				tsm.init_name as tsm_id,
				DATE_FORMAT(NOW()+ INTERVAL 3 MONTH, '%d/%m/%Y') as EXPDate,
				'' as EXPCard,
				prd.ProductCode AS product_id,
