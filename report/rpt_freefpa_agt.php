...
//
	...
	function followup(){
		...
		$sql =" select cmp.CampaignNumber as cmpnum,
				...
				date(cmp.CampaignEndDate) as cmpenddate,
				date(cmp.CampaignExtendedDate) as cmpextdate
	...
	function callattempt(){
		...
		$sql =" select cmp.CampaignNumber as cmpnum,
				sum(if(clh.CallHistoryId>1,1,0)) as attempted
				from t_gn_callhistory clh
				left join t_gn_customer cst on cst.CustomerId = clh.CustomerId
				left join t_gn_campaign cmp on cmp.CampaignId = cst.CampaignId
				where date(clh.CallHistoryCallDate) BETWEEN '".$_REQUEST['start_date']."' AND '".$_REQUEST['end_date']."'
				GROUP BY cmp.CampaignNumber ";
...
<legend style="color: teal;"> &nbsp;&nbsp;&nbsp;Preview RCT (Per Campaign) - Closing QA&nbsp;&nbsp;&nbsp;</legend>
...
<thead>
	...
	<tr>
					...
					<th class="head" bgcolor="teal" style="color:#AFA;text-align:center;" align="center" colspan="2" nowrap >Fax</th>
...
<tbody>
	<?php
		...
		while($row = $db ->fetchrow($ListPages->result)){
			...
			$totalfax = 0;
			$totalfax[$row -> campaignnumber] += $row -> Fax;
			...
			$persenflw [$row -> campaignnumber]= (($slc_count[$row->campaignnumber])/($cmprow_count[$row->campaignnumber]))*100;
			
			
			$persenInvalidPhoneNumber [$row -> campaignnumber]= (($row -> InvalidPhoneNumber) / ($slc_count[$row->campaignnumber]))*100;
			$persenBusyLine [$row -> campaignnumber]= (($row -> BusyLine)/($slc_count[$row->campaignnumber]))*100;
			$persenNobodyPicksUp [$row -> campaignnumber]= (($row -> NobodyPicksUp) / ($slc_count[$row->campaignnumber]))*100;
			$persenMissCustomer [$row -> campaignnumber]= (($row -> MissCustomer) / ($slc_count[$row->campaignnumber]))*100;
			$persenChangeOwner [$row -> campaignnumber]= (($slc_count[$row->campaignnumber]) / $row -> supply)*100;
			$persenInterestedWithSpouse [$row -> campaignnumber]= (($slc_count[$row->campaignnumber])/($row -> supply))*100;
			//
			$ratio [$row -> campaignnumber]= $attempt_count[$row->campaignnumber] /$slc_count[$row->campaignnumber] ;
			$persenslc[$row->campaignnumber]= (($row -> attmp)/($slc_count[$row->campaignnumber]))*100;
			$persenFax [$row -> campaignnumber]= $row -> Fax /($slc_count[$row->campaignnumber]);
			//
			$totalconnected [$row -> campaignnumber]=($slc_count[$row->campaignnumber]) - ($row -> Fax) - ($row -> InvalidPhoneNumber);
			$totalpersenconnected [$row -> campaignnumber]=(($totalconnected [$row -> campaignnumber])/($slc_count[$row->campaignnumber]))*100;
			$totalcontacted [$row -> campaignnumber]=(($totalconnected [$row -> campaignnumber]))-($row -> BusyLine)-($row -> NobodyPicksUp)-($row -> MissCustomer);
			$totalpersencontacted [$row -> campaignnumber]=(($totalcontacted [$row -> campaignnumber])/($slc_count[$row->campaignnumber]))*100;
			...
			$RR [$row -> campaignnumber] = ($pif [$row -> campaignnumber] / $slc_count[$row->campaignnumber])*100;
	...
	?>
			...
			<tr>
				...
				<td nowrap style="text-align: center" class="content-middle"><?php echo $cmprow_count[$row->campaignnumber]; ?></td>
				<td nowrap style="text-align: center" class="content-middle"><?php echo $slc_count[$row->campaignnumber]; ?></td>
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format($persenflw [$row -> campaignnumber],2) ; ?></td>
				<td nowrap style="text-align: center" class="content-middle"><?php echo $attempt_count[$row->campaignnumber];?></td>
	...
	<?php
		$no++;
		$grandtotalfax = 0;
		$grandtotalfax[$row -> campaignnumber] += $totalfax[$row -> campaignnumber];
		};
			
	?>
	<!--<tr>
	<td bgcolor="teal" style="color:#AFA;text-align:center;" class="footer-first" colspan="4"><?php echo "T O T A L"; ?></td>
	<td nowrap style="text-align: center" class="footer-middle"><?php echo $grandtotalfax[$row -> campaignnumber] ;?></td>
	</tr>-->
