		...
		function getCases($CampaignId,$UploadId,$cat=""){
			switch($cat){
				case "cases":
					...
					break;
				case "contacted":
					$sql = "select count(*) from (select a.CustomerId,
							...
							left join t_gn_customer b ON a.CustomerId = b.CustomerId
							left join t_gn_campaign c ON b.CampaignId = c.CampaignId
							where true  and b.CampaignId in (".$CampaignId.")
							and a.CallHistoryCallDate >= '".$_REQUEST['start_date']." 00:00:00'
							and a.CallHistoryCallDate <= '".$_REQUEST['end_date']." 23:00:00'
							group by a.CustomerId having Contact is not null) as Cases;
							";
							// echo "<pre>".$sql."</pre>";
					break;
				case "nottouch":
