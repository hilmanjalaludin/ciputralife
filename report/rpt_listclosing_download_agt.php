<?php
	...
	$agt			= $_REQUEST['Agent'];
	$excel			= new excel();
	$excel -> xlsWriteHeader('DownloadListClosing(ClosingAgent)(From'.$start_date.'To'.$end_date.')');
	...
	$excel ->xlsWriteLabel(0,25,"Policy No");
	$excel ->xlsWriteLabel(0,26,"Call Reason ID");
	$excel ->xlsWriteLabel(0,27,"Call Reason Desc");
...
//	// Mendefinisikan nama file ekspor "hasil-export.xls"z
	//header("Content-Disposition: attachment; filename=".($filename));
		...
		while($row = $db ->fetchrow($query)){
				...
				$excel->xlsWriteLabel($xlsRows,23,$row -> Agentid );
				$excel->xlsWriteLabel($xlsRows,24,$row -> Agent_Name );
				$excel->xlsWriteLabel($xlsRows,25,$row -> policyno );
				$excel->xlsWriteLabel($xlsRows,26,$row -> callreasonid );
				$excel->xlsWriteLabel($xlsRows,27,$row -> callreasondesc );
