...
function CreateReport(){
	...
	$sql = "SELECT a.CallReasonId,b.CallReasonCode,c.CampaignId,
			...
			WHERE 1=1
			AND a.CustomerUpdatedTs >= '".$start_date." 00:00:00' 
			AND a.CustomerUpdatedTs <= '".$end_date." 23:00:00'
			AND c.CampaignStatusFlag = 1";
		$sql.=" GROUP BY a.CallReasonId,a.CampaignId
	...
	$sqlnew = 	"SELECT 'New' as CallReasonId,'New' as CallReasonCode, IF(b.CallReasonContactedFlag IS NULL,2,b.CallReasonContactedFlag) AS CallReasonContactedFlag, IF(b.CallReasonDesc IS NULL,'New',b.CallReasonDesc) AS CallReasonDesc, c.CampaignNumber,c.CampaignName, COUNT(a.CustomerId) AS TOT
				FROM t_gn_customer a
				LEFT JOIN t_lk_callreason b ON a.CallReasonId=b.CallReasonId
				LEFT JOIN t_gn_campaign c ON a.CampaignId=c.CampaignId
				WHERE 1=1 AND c.CampaignStatusFlag = 1
	...
	// $fp = fopen(dirname(__FILE__).'/Generated/Report_CampaignReview_'.$Reporttype.'_'.date("Ymd").'.xls', "w");
	$fp = fopen('/opt/enigma/webapps/ciputralife/report/Generated/Report_CampaignReview_'.$Reporttype.'_'.date("Ymd").'.xls', "w");
