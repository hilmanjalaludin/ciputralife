...
if($rawdata_type == 1) {	// raw data daily
	$fileName = 'EXTRACT_DAILY_NONACTIVE_'.date("Ymd");
	$judul = 'DAILY NONACTIVE '.date("Ymd");
	$start_date = date("Y-m-d 01:00:00");
	$end_date = date("Y-m-d 23:00:00");
}else {						// MTD
	$fileName = 'EXTRACT_MTD_NONACTIVE_'.date("Ymd");
	$judul = 'MTD NONACTIVE '.date("Ymd");
