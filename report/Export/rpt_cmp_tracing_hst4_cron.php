<?php
	...
	function CreateReportTracing()
	{
		...
		/*********************CAMPAIN INFO*******************/
		// if($MYSQLConnect->havepost('Campaign'))
		// {
			$sql = "select
					b.UserId as mgrid,
					b.id as mgr,
					...
					";
			// if($AssignSelerId != "") {
			// 	$sql .= " and si.AssignSelerId IN (".$AssignSelerId.")";
			// }
			// if($spv_id != "") {
			// 	$sql .= " and a.spv_id IN (".$spv_id.")";
			// }
			...
			{
				$CampaignInfo[$rows['mgrid']]['mgrid'] = $rows['mgrid'];
				$CampaignInfo[$rows['mgrid']]['mgr'] = $rows['mgr'];
				$CampaignInfo[$rows['mgrid']]['mgrname'] = $rows['mgrname'];
				$CampaignInfo[$rows['mgrid']]['datasize'] = $rows['datasize'];
			...
			// if($AssignSelerId != "")
			// {
			// 	$sql .= " and a.SellerId IN (".$AssignSelerId.")";
			// }
			// if($spv_id != "")
			// {
			// 	$sql .= "and h.spv_id IN (".$spv_id.")";
			// }
			...
			foreach($qry -> result_assoc() as $rows )
			{
				...
				$CasesAPE[$rows['mgrid']]['APE'] += (int)$rows['APE'];
				$CasesAPE[$rows['mgrid']]['PREMI'] += (int)$rows['PREMI'];
				$CasesAPE[$rows['mgrid']]['CASES'] += (int)$rows['CASES'];
			}
			/**********************************************************************/
			...
			foreach($qry -> result_assoc() as $rows )
			{
				...
				$summaryReason[$rows['mgrid']]['contacted'] += (int)$rows['contacted'];
				$summaryReason[$rows['mgrid']]['uncontacted'] += (int)$rows['uncontacted'];
			...
			foreach($qry -> result_assoc() as $rows )
			{
				$ReasonCall[$rows['mgrid']]['BUSY'] += (INT)$rows['BUSY'];
				$ReasonCall[$rows['mgrid']]['INVALID_NUMBER'] += (INT)$rows['INVALID_NUMBER'];
				$ReasonCall[$rows['mgrid']]['NO_PICK'] += (INT)$rows['NO_PICK'];
				$ReasonCall[$rows['mgrid']]['CALL_AGAIN'] += (INT)$rows['CALL_AGAIN'];
				$ReasonCall[$rows['mgrid']]['MISS_CUSTOMER'] += (INT)$rows['MISS_CUSTOMER'];
				$ReasonCall[$rows['mgrid']]['THINKING'] += (INT)$rows['THINKING'];
				$ReasonCall[$rows['mgrid']]['MOVED'] += (INT)$rows['MOVED'];
				$ReasonCall[$rows['mgrid']]['OVERAGE'] += (INT)$rows['OVERAGE'];
				$ReasonCall[$rows['mgrid']]['DONOTCALL'] += (INT)$rows['DONOTCALL'];
				$ReasonCall[$rows['mgrid']]['NO_CARD'] += (INT)$rows['NO_CARD'];
				$ReasonCall[$rows['mgrid']]['NOT_INTEREST'] += (INT)$rows['NOT_INTEREST'];
				$ReasonCall[$rows['mgrid']]['WRONG_PERSON'] += (INT)$rows['WRONG_PERSON'];
				$ReasonCall[$rows['mgrid']]['FOLLOWUP_EMAIL'] += (INT)$rows['FOLLOWUP_EMAIL'];
				$ReasonCall[$rows['mgrid']]['FOLLOWUP_WA'] += (INT)$rows['FOLLOWUP_WA'];
				$ReasonCall[$rows['mgrid']]['SALES'] += (INT)$rows['SALES'];
				$ReasonCall[$rows['mgrid']]['Thinking_WA'] += (INT)$rows['Thinking_WA'];
				$ReasonCall[$rows['mgrid']]['Thinking_Email'] += (INT)$rows['Thinking_Email'];
				$ReasonCall[$rows['mgrid']]['Agree_WA'] += (INT)$rows['Agree_WA'];
				$ReasonCall[$rows['mgrid']]['Agree_Email'] += (INT)$rows['Agree_Email'];
				$data[$rows['mgrid']]['Reject_Email'] += (INT)$rows['Reject_Email'];
				$ReasonCall[$rows['mgrid']]['Reject_WA'] += (INT)$rows['Reject_WA'];
				$ReasonCall[$rows['mgrid']]['Touch'] += (INT)$rows['Touch'];
			}
			/**********************************************************************/
			...
			// if($AssignSelerId != "")
			// {
			// 	$sql .= " and b.CreatedById IN (".$AssignSelerId.")";
			// }
			// if($spv_id != "")
			// {
			// 	$sql .= " and c.spv_id IN (".$spv_id.")";
			// }
			...
			foreach($qry -> result_assoc() as $rows )
			{
				...
				$attempt[$rows['mgrid']]['CallAttempt'] = $rows['CallAttempt'];
				...
				$objPHPExcel->setActiveSheetIndex(0)
					//judul
					->mergeCells('A1:J1')
					->setCellValue('A1','Report Tracking Review History 4')
				...
				$objPHPExcel->setActiveSheetIndex(0)
					...
					->mergeCells('B3:B4')
					->setCellValue('B3','MGR ID')

					->mergeCells('C3:C4')
					->setCellValue('C3','MGR Name')
				...
				foreach($CampaignInfo as $up_id => $arr_val)
				{
					...
					$objPHPExcel->getActiveSheet()->setCellValueExplicitByColumnAndRow($coloms, $rows, $arr_val['mgr']);
					
					$coloms++;
					$objPHPExcel->getActiveSheet()->setCellValueExplicitByColumnAndRow($coloms, $rows, $arr_val['mgrname']);
			...
			if($Modes[1]=="MTD"){ // MTD
				$outputFile = "R_C_TrackingHstMTD4_".date('Ymd');
			}else{ // DAILY
				$outputFile = "R_C_TrackingHstDAILY4_".date('Ymd');
			}
