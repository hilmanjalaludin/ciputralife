...
$objPHPExcel->getActiveSheet()->SetCellValue('A1', 'Campaign Overview');
...
$objWriter->save("/opt/enigma/webapps/ciputralife/report/Generated/ReportCampaignOverview_".$reporttype."-".date('Ymd').".xlsx");
	...
	$sql = "SELECT COUNT(DISTINCT tcst.CustomerId) as Cases FROM t_gn_campaign tcmp
			...
			switch($cat){
				case "cases":
					...
					$sql.=" AND tcst.CallReasonId = 15 AND tcst.CallReasonQue = 1 AND DATE(tcst.CustomerUpdatedTs) >= '".$start_date." 00:00:00'
					AND DATE(tcst.CustomerUpdatedTs) <= '".$end_date." 23:00:00';";
					break;
				case "contacted":
					// $sql.=" AND tcll.CallReasonCategoryId IN (2,3,4)";
					$sql.=" AND tcll.CallReasonContactedFlag = 1 AND DATE(tcst.CustomerUpdatedTs) >= '".$start_date." 00:00:00'
					AND DATE(tcst.CustomerUpdatedTs) <= '".$end_date." 23:00:00';";
					break;
				case "nottouch":
					...
					break;
				case "touch":
					$sql.=" AND tcst.CallReasonId IS NOT NULL AND DATE(tcst.CustomerUpdatedTs) >= '".$start_date." 00:00:00'
					AND DATE(tcst.CustomerUpdatedTs) <= '".$end_date." 23:00:00';";
					break;
			}
