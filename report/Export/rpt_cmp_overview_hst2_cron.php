...
$objPHPExcel->getActiveSheet()->SetCellValue('A1', 'Campaign Overview By History 2');
...
$objWriter->save("/opt/enigma/webapps/ciputralife/report/Generated/ReportCampaignOverviewHistory2_".$reporttype."-".date('Ymd').".xlsx");
...
function getCases($CampaignId,$UploadId,$cat=""){
	...
	case "cases":
		...
		left join t_gn_campaign cmp ON tcst.CampaignId = cmp.CampaignId
		...
		WHERE 1=1
		AND tcst.CampaignId in (".$CampaignId.")
	...
	case "contacted":
		$sql = "select count(*) from (select a.CustomerId,
					(select max(ax.CallHistoryId) from t_gn_callhistory ax
						...
						and ax.CallHistoryCallDate <= '".$end_date." 23:00:00') as `Contact`
						#,
				...
				and a.CallHistoryCallDate <= '".$end_date." 23:00:00'
	...
	case "nottouch":
		...
		WHERE 1=1
		AND tcmp.CampaignId in (".$CampaignId.")
	...
	case "touch":
		$sql = "select count(distinct ch.CustomerId) as Cases
			...
			AND cmp.CampaignId in (".$CampaignId.")
			";
			// echo "<pre>".$sql."</pre>";
		break;
	}
