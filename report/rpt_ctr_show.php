...
//
	...
	function asgn(){
		...
		$sql = " select cmp.campaignnumber AS campaignnumber,
				...
				sum(if(cst.CallReasonId is not null,1,0)) as solicited,
				sum(if(clh.CallHistoryId is not null,1,0)) as attmp,
				sum(if(asg.AssignSelerId is not null,1,0)) as assign,
				...
				sum(if(cst.CallReasonId =16,1,0)) as Interested,
				sum(if(cst.CallReasonId =17,1,0)) as InterestedWithSpouse,
				...
				sum(if(cst.CallReasonId =40,1,0)) as NoNeedInsurance,
				sum(if(cst.CallReasonId =41,1,0)) as ThinkingAddNewNumber,
				sum(if(cst.CallReasonId =42,1,0)) as NotInterestedOther,
				...
				AND date(clh.CallHistoryCallDate) BETWEEN '$start_date' AND '$end_date'
...
<legend style="color: teal;"> &nbsp;&nbsp;&nbsp;Preview RCT (Per Campaign)&nbsp;&nbsp;&nbsp;</legend>
...
<tbody>
	<?php
		...
		while($row = $db ->fetchrow($ListPages->result)){
			...
			$persenflw [$row -> campaignnumber]= (($row -> solicited)/($cmprow_count[$row->campaignnumber]))*100;
			
			
			$persenInvalidPhoneNumber [$row -> campaignnumber]= (($row -> InvalidPhoneNumber) / ($row -> solicited))*100;
			$persenBusyLine [$row -> campaignnumber]= (($row -> BusyLine)/($row -> solicited))*100;
			$persenNobodyPicksUp [$row -> campaignnumber]= (($row -> NobodyPicksUp) / ($row -> solicited))*100;
			$persenMissCustomer [$row -> campaignnumber]= (($row -> MissCustomer) / ($row -> solicited))*100;
			$persenChangeOwner [$row -> campaignnumber]= (($row -> solicited) / $row -> supply)*100;
			$persenInterestedWithSpouse [$row -> campaignnumber]= (($row -> solicited)/($row -> supply))*100;
			//
			$ratio [$row -> campaignnumber]= $row -> attmp /$row -> solicited ;
			$persenslc[$row->campaignnumber]= (($row -> attmp)/($row -> solicited))*100;
			$persenFax [$row -> campaignnumber]= $row -> Fax /($row -> solicited);
			//
			$totalconnected [$row -> campaignnumber]=($row -> solicited) - ($row -> Fax) - ($row -> InvalidPhoneNumber);
			$totalpersenconnected [$row -> campaignnumber]=(($totalconnected [$row -> campaignnumber])/($row -> solicited))*100;
			$totalcontacted [$row -> campaignnumber]=(($totalconnected [$row -> campaignnumber]))-($row -> BusyLine)-($row -> NobodyPicksUp)-($row -> MissCustomer);
			$totalpersencontacted [$row -> campaignnumber]=(($totalcontacted [$row -> campaignnumber])/($row -> solicited))*100;
			...
			$RR [$row -> campaignnumber] = ($pif [$row -> campaignnumber] / $row -> solicited)*100;
	...
	?>
			...
			<tr>
				...
				<td nowrap style="text-align: center" class="content-middle"><?php echo $row -> solicited; ?></td>
				<td nowrap style="text-align: center" class="content-middle"><?php echo number_format($persenflw [$row -> campaignnumber],2) ; ?></td>
				<td nowrap style="text-align: center" class="content-middle"><?php echo $row -> attmp;?></td>
