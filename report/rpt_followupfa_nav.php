	...
	<script>
		$('document').ready(function() {
			...
			$('#start_date').datepicker({
				showOn: 'button',
				buttonImage: '<?php echo $app->basePath();?>gambar/calendar.gif',
		...
		//http://192.168.16.6/devel/report/rpt_cmp_overview_hst2.php?action=ShowReport&start_date=2018-03-01&end_date=2018-03-26&rpttype=cmp.overviewhst2&cmp=1
		
		function ShowReports($CignaSystemCode) {
			//alert('inialert');return  false;
			...
			if(start_date !=0 & end_date !=0) {
				if(rpttype =='cmp.FA')  {
					var url  = "../report/rpt_fa_view.php?action=ShowReport&start_date="+start_date+"&end_date="+end_date+"&rpttype="+rpttype+"&cmp="+cmp;
						window.open(url,'_blank');
					}
					if(rpttype =='cmp.TNAS')  {
					var url  = "../report/rpt_tna_view.php?action=ShowReport&start_date="+start_date+"&end_date="+end_date+"&rpttype="+rpttype+"&cmp="+cmp;
						window.open(url,'_blank');
					}
					if(rpttype =='cmp.MTF')  {
					var url  = "../report/rpt_mtf_view.php?action=ShowReport&start_date="+start_date+"&end_date="+end_date+"&rpttype="+rpttype+"&cmp="+cmp;
						window.open(url,'_blank');
					}
					return;
				}else
		...
		function GetReport($CignaSystemCode){
			...
			if(start_date !=0 & end_date !=0) {
				if(rpttype =='cmp.FA')  {
					var url  = "../report/rpt_fa_download.php?action=ShowReport&start_date="+start_date+"&end_date="+end_date+"&rpttype="+rpttype+"&cmp="+cmp;
						window.open(url,'addressbar=no');
					}
					if(rpttype =='cmp.TNAS')  {
					var url  = "../report/rpt_tna_download.php?action=ShowReport&start_date="+start_date+"&end_date="+end_date+"&rpttype="+rpttype+"&cmp="+cmp;
							window.open(url,'addressbar=no');
							
					}
					if(rpttype =='cmp.MTF')  {
					var url  = "../report/rpt_mtf_download.php?action=ShowReport&start_date="+start_date+"&end_date="+end_date+"&rpttype="+rpttype+"&cmp="+cmp;
							window.open(url,'addressbar=no');
						//alert(url);
					}
					return;
...
<div>
	...
	<fieldset class="corner">
		<legend class="icon-product"> &nbsp;&nbsp;&nbsp;Form Report Follow Up FA</legend>
			...
			<div id="rpt_top_content" class="box-shadow" style="width:99%;height:auto;overflow:auto;">
						...
						<table cellpadding="5px" align="left">
							...
							<tr>
								...
								<select style="width:220px" name="rpttype" id="rpttype">
									...
									<option value="cmp.FA">Follow FA</option>
									<option value="cmp.TNAS">Follow TNA </option>
									<option value="cmp.MTF">Follow MTF</option>
								</select>
							<tr>
							<tr>
				...
				<a href="javascript:void(0);" style="float:right;margin-right:50px;margin-bottom:20px;" class="sbutton" onclick="ShowReports();"><span>Show Report</span></a>
			</div>
			</div>
	</fieldset>
</div>