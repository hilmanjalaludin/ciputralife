...
class index extends mysql
{
	...
	function header()
	{
		global $Themes;
		$report_type = array (
			'cmp_tracking_overview' => 'CampaignOverview',
			'cmp_tracking_overview2' => 'CallTrackingbyAgent',
			'cmp_tracking_overview3' => 'ReportCallTrackingBySPV',
			'cmp_tracking_overview4' => 'ReportCallTrackingByMGR'
			);
			// echo ;
		$labelReport  		 = $report_type[$this -> escPost('report_type')];
		
		$xlsName = $labelReport.'_'.date('Ymd').'_'.date('His').'.xls';
	...
	private function setLabel()
	{
		$report_type = array (
			...
			'cmp_tracking_overview2' => 'Call Tracking by Agent',
		...
		echo "<div class=\"label_header\" style=\"margin-bottom:5px;padding-top:5px;padding-bottom:5px;border-bottom:1px solid #eee;width:'100%';\">
				...
				<span class='middle'>Interval Date : {$start_date} - {$end_date}  </span> |
