...
class Form extends mysql{
	...
	{
		$list = "<fieldset style=\"background-color:#FFFFCC;border:1px solid #eeeeee;\">
	...
	{
		$radio = "<fieldset style=\"background-color:#FFFFCC;border:1px solid #eeeeee;\">
				...
				foreach($data as $key => $value )
				{
					$radio .= "<tr>";
					if($attr){
						$radio .= "<td style=\"border-bottom:1px solid #eeeeee;\"><input type=\"radio\" ".($dis?'disabled':'')." id=\"".($name==''?'txtFrm':$name)."\" name=\"".($name==''?'txtFrm':$name)."\" class=\"".($css==''?'default':$css)."\" ".($js==''?'':$js)." value=\"".($key==''?'':$key)."\" checked=\"true\">&nbsp;".($value==''?'':$value)."</td>";
					}
					else{
						if( $checked == $key )
						{
						...
						{
							$radio .= "<td style=\"border-bottom:1px solid #eeeeee;\"><input type=\"radio\" ".($dis?'disabled':'')." id=\"".($name==''?'txtFrm':$name)."\" name=\"".($name==''?'txtFrm':$name)."\" class=\"".($css==''?'default':$css)."\" ".($js==''?'':$js)." value=\"".($key==''?'':$key)."\">&nbsp;".($value==''?'':$value)."</td>";
						}
					}
					$radio .= "</tr>";
	...
	public function RTInput($name="txtFrm",$css="",$value="",$label=array(),$js="",$true=false,$maxLength=100)
	{
		...
		{
			$textbox = "<fieldset style=\"background-color:#FFFFCC;border:1px solid #eeeeee;\">
				
				<div style=\"height:30px;overflow:auto;background-color:#FFFFCC;border:0px solid #eeeeee;\">
					<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"90%\">";
