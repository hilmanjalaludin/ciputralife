...
Ext.DOM.CopyDataPayer2Holder = function(opt) {
	(opt.checked ? (
		...
		// Ext.Cmp('HolderAddrType').setValue(Ext.Cmp('PayerIdentificationTypeId').getValue()),
...
Ext.DOM._get_result_insured = function() {
	...
	else if (Ext.Cmp("InsuredPremi").getValue()=='0' || Ext.Cmp("InsuredPremi").getValue()=='') {
        alert('Invalid Premi');
        next_process = 0;
    }
...
Ext.DOM.SavePolis = function() {
    ...
    // if (Ext.DOM.validInsured()) {
		...
		// {
			...
			else if (!Ext.DOM._get_result_holder()) {
				return false;
			}
			else if (Ext.Cmp("Benefeciery").Checked()==0) {
				alert('Beneficiary required');
				return false;
			}
			else if (!Ext.DOM.ValidasiBenef()) {
				alert("Beneficiary not complete!");
				return false;
			}
			// else if (!Ext.DOM.ValidasiPayment()) {
