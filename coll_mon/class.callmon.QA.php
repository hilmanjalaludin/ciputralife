...
class CallmonQA extends mysql
{
	...
	**/
	...
	function getCustomerInfo(){
		$sql = "SELECT a.CustomerId,a.CustomerFirstName, b.CampaignName, d.ProductCode, d.ProductName,
				DATE_FORMAT(f.PolicySalesDate,'%d-%m-%Y') AS SELLINGDATE, g.id, g.Full_Name, sum(h.duration) as Duration
				...
				LEFT JOIN cc_recording h ON a.customerid = h.assignment_data
	...
	function _get_link_jam()
	{
		$arr_2dimensi = array();
		$sql = " select b.LinkPointId, b.minutes as minutes, b.seconds
		...
		//echo $sql;
		$qry = mysql_query($sql);
		if( $qry )while( $row = mysql_fetch_assoc($qry) ){
			$arr_2dimensi[$row['LinkPointId']]['minutes'] = $row['minutes'];
			$arr_2dimensi[$row['LinkPointId']]['seconds'] = $row['seconds'];
		}
		return (array)$arr_2dimensi;
	}	
 ...
 function getRemarkExtra()
 {
	...
	**/
			...
			if( $_ReportId )
			{
				if( self::_delete_link_point($_CustomerId) )
				{
					...
					foreach( $qry -> result_assoc() as $rows )
					{
						...
						$menit  = $this ->escPost('menit_'.$rows['CollCategoryId'].'_'.$rows['SubCategoryId']);
						$detik  = $this ->escPost('detik_'.$rows['CollCategoryId'].'_'.$rows['SubCategoryId']);
						if( $this -> havepost($V_POST) )
						{
							...
							$V_POINT['Minutes']	 = $menit;
							$V_POINT['Seconds']	 = $detik;
	...
	function SaveScore()
	{
		...
		$InsReport['Total_Courtesy'] = $this -> escPost('courtesy_1') + $this -> escPost('courtesy_3');
		$InsReport['Score_Courtesy'] = $this->escPost('courtesy_2') + $this -> escPost('courtesy_4'); 
...
?>
		<table cellpadding="3px" style="margin-bottom:4px;" width="99%" border="0">
			...
			<tr>
				...
				<tr>
					...
					<td>:</td>
					<td><?php $this -> Form -> jpInput('CustomerId','input',$_Customer['CustomerId'],null,1); ?></td>
				</tr>
				<tr>
					<td nowrap> Agent Id </td>
					<td>:</td>
					<td><?php $this -> Form -> jpInput('customer','input',$_Customer['Full_Name'],null,1); ?></td>
					<td nowrap>Duration</td>
					<td>:</td>
					<td><?php $this -> Form -> jpInput('product_name','input',toDuration($_Customer['Duration']),null,1); ?></td>
				</tr>
		...
		{ ?>
			<table style="font-size:10px;" border="0" width="70%" cellpadding="4px" cellspacing="1">
				...
				<tr>
					<td style="font-size:11px;color:red;" class="content-text" nowrap>Total Courtesy 1</td>
					<td style="font-size:11px;color:red;" class="content-text" nowrap>:</td>
					...
					<td style="font-size:11px;color:red;" class="content-text" nowrap>Total Courtesy 2</td>
					<td style="font-size:11px;color:red;" class="content-text" nowrap>:</td>
					<td style="font-size:11px;color:red;" class="content-text" nowrap><?php $this -> Form->jpInput('courtesy_3','overall_point','0',NULL,1);?></td>
					<td style="font-size:11px;color:red;" class="content-text" nowrap><?php $this -> Form->jpInput('courtesy_4','overall_point','0',NULL,1);?></td>
				<tr>
	...
	function getCallContent()
	{
		...
		foreach($Category as $key => $value){
			...
			// $minute  = ( isset($jam[$_link_point_id]['minutes']) ? $jam[$_link_point_id]['minutes'] :0);
			// $second  = ( isset($jam[$_link_point_id]['seconds']) ? $jam[$_link_point_id]['seconds'] :0);
			$minute  = ( isset($jam[$_link_point_id]['minutes']));
			$second  = ( isset($jam[$_link_point_id]['seconds']));
			?>
				<tr class="select-rows">
					...
					<!--<td style="font-size:11px;color:red;" class="content-text" nowrap><?php $this -> Form->jpInput("menit_".$CategoryId[CategoryId]."_".$rows[SubCategoryId],'box',$jam[$_link_point_id]['minutes'],null)?></td>-->
					<td style="font-size:11px;color:red;" class="content-text" nowrap>
					<input type='text' class="box" id="menit_<?php echo $CategoryId[CategoryId]."_".$rows[SubCategoryId]?>" name="menit_<?php echo $CategoryId[CategoryId]."_".$rows[SubCategoryId]?>" value="<?php echo $minute ?>" placeholder="00" maxlength="2">
					</td>
					<td style="font-size:11px;color:red;" class="content-text" nowrap>:</td>
					<td style="font-size:11px;color:red;" class="content-text" nowrap>
					<input type='text' class="box" id="detik_<?php echo $CategoryId[CategoryId]."_".$rows[SubCategoryId]?>" name="detik_<?php echo $CategoryId[CategoryId]."_".$rows[SubCategoryId]?>" value="<?php echo $second ?>" placeholder="00" maxlength="2">
					</td>
					<!--<td style="font-size:11px;color:red;" class="content-text" nowrap><?php $this -> Form->jpInput("detik_".$CategoryId[CategoryId]."_".$rows[SubCategoryId],'box',$second,null) ?></td>-->
