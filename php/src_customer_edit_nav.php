<?php
 ...
 /** get Call status list **/
	...
	{
		global $db;
		$sql = "select a.CallReasonId, a.CallReasonCode, a.CallReasonDesc from t_lk_callreason a  where a.CallReasonStatusFlag=1 order by a.CallReasonId asc";
...
/** not valid page if not search **/
 ...
 /** set filter **/
	...
	$filter =  " AND b.AssignAdmin is not null 
				 ...
				 AND f.CallReasonCategoryId IN(".getClsoingStatus().")
				 AND b.AssignBlock=0 
...
?>
	...
	/* assign navigation filter **/
		...
		var navigation = {
			custnav:'src_customer_edit_nav.php',
			custlist:'src_customer_edit_list.php'
		}
	...
	/* creete object javaclass **/
		...
		doJava.File = '../class/class.src.edit.customers.php' 
 ...
 /* go to call contact detail customers **/
		...
		var EditPolicy = function(){
			var arrCallRows  = doJava.checkedValue('chk_edit_policy');
			var arrCountRows = arrCallRows.split(','); 
				if( arrCallRows!='')
				{	
					if( arrCountRows.length == 1 )
					{
						doJava.File = 'dta_edit_policy.php';
	...
	/* memanggil Jquery plug in */
		...
		$(function(){
			...
			$('#toolbars').extToolbars({
				extUrl   :'../gambar/icon',
				extTitle :[['Search'],['Edit Poliicy '],['Clear']],
				extMenu  :[['searchCustomer'],['EditPolicy'],['resetSeacrh']],
				extIcon  :[['zoom.png'],['folder_edit.png'],['cancel.png']],
				extText  :true,
				extInput :true,
				extOption:[{
						render : 4,
	...
	<!-- start : content -->
		...
		<fieldset class="corner">
			<legend class="icon-customers">&nbsp;&nbsp;Customer Edit Policy </legend>	
