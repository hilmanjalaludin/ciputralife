...
class rpt_summary_freepa extends mysql {
	function getspv() {
		$datas = array();
	...
	function getAM(){
		$datas = array();
		if($this -> getSession('handling_type')==USER_ACCMANAGER){
			$sql = "select a.UserId as mgrid, concat(a.id,' - ',a.full_name) as spvname
					from tms_agent a
					where a.UserId = ".$this -> getSession('UserId')." and a.user_state = 1 order by a.full_name";
			...
			$i = 1;
			while ($row = $this -> fetchassoc($qry)){
				$result[$row['mgrid']] = $row['spvname'];
				$i++;
			}
		}else if ($this -> getSession('handling_type') <= 1 OR $this -> getSession('handling_type') == 9){
		
			$sql = "select a.UserId as manager_id, concat(a.id,' - ',a.full_name) as spvname
					from tms_agent a
					where a.mgr_id = a.UserId and a.user_state = 1 order by a.full_name";

			$qry = $this -> execute($sql, __FILE__, __LINE__);
			$i = 1;
			while ($row = $this -> fetchassoc($qry)){
				$result[$row['manager_id']] = $row['spvname'];
				$i++;
			}
		}
		return $result;
...
var ShowHTML  = function ()
{
	var dialog = Ext.Window({
            url: '../report/index.html.sum_freepa.php',
            ...
            param: {
                ...
                start_selling_date : Ext.Cmp('start_selling_date').getValue(),
                end_selling_date : Ext.Cmp('end_selling_date').getValue(),
...
var ShowExcel=function ()
{
	...
	var dialog = Ext.Window({
            url: '../report/index.excel.free_fpa.php',
            name: 'WinRptCampTrack',
            param: {
                report_type: Ext.Cmp('report_type').getValue(),
                start_date: Ext.Cmp('start_date').getValue(),
...
$(function(){
	...
	$('#start_selling_date').datepicker({showOn: 'button', buttonImage: '../gambar/calendar.gif', buttonImageOnly: true, dateFormat:'dd-mm-yy',readonly:true});
	$('#end_selling_date').datepicker({showOn: 'button', buttonImage: '../gambar/calendar.gif', buttonImageOnly: true, dateFormat:'dd-mm-yy',readonly:true});
});
</script>
<style>
	...
	<div id="span_top_nav">
		<table cellpadding="4px">
			...
			<tr>
				...
				<td>
					<?php $jpForm->jpInput('start_selling_date','xx002');?> &nbsp;-&nbsp;
					<?php $jpForm->jpInput('end_selling_date','xx002');?>
				</td>
			</tr>
			<tr>
				<td style="font-size:12px;padding-right:12px;font-family:Arial;color:red;text-align:right;" valign="top"><span id="label_segment2_cigna">AM</span></td>
				<td> <span id="List_spv"> <?php $jpForm->jpCombo('group_filter_segment1','xx002',$aInit->getAM(),NULL);?></span> </td>
			</tr>
			<tr>
