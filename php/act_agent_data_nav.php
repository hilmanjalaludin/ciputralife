<?php
	...
	// define('USER_SUPERVISOR','4');
	// define('USER_QUALITY','5');
...
// get call reaon 
	function getCallReason()
	{
		...
		$sql = "select a.CallReasonId, a.CallReasonDesc from t_lk_callreason a 
				...
				AND a.CallReasonId NOT IN (13,14,15,21,22) ";
		$qry = $this -> query($sql);
	...
	function getSPVDatas()
		{
			...
			switch( USER_LEVEL )
			{
				...
				case USER_ROOT: 
					$filter = " AND a.handling_type ='".USER_SUPERVISOR."'"; 
				break;
...
/* ** show data for assignment ****/
	...
	var ShowDatas = function()
	{
		...
		var Spvname		= doJava.dom('ToSupervisorId').value;
...
//////////////////////////////////////////////////////////
	...
	/// 28-04-2014 get Agent per SPV
	var ToAgentBySpv = function(combo)
	{
		var UserLevel = combo.value;
	...
	<legend class="icon-customers">&nbsp;&nbsp;Get Agent Data </legend>	
		<fieldset  style="margin-top:6px;border:1px solid #ddd;">
			...
			<table cellpadding='7px'>
				<tr>
					<td></td><td></td>
					<td class="text_header2" valign="top" nowrap>SPV Name</td>
					<td><?php echo $jpForm ->jpCombo('ToSupervisorId', 'drop_dwn',$Distribusi->getListToSpv(),$db -> getSession('spv_id'),'onChange="ToAgentBySpv(this);"',($db -> getSession('spv_id')?1:0));?></td>
					<td></td><td></td>
				</tr>
				<tr>
					...
					<td id="toSellerId"><?php echo $jpForm ->jpListcombo('agent_name','Select',$Distribusi ->getUserDatas());?></td>
