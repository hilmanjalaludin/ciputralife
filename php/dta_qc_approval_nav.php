...
/** set general query SQL ****/
	...
	if($db->escPost('aksi')=="deletion"){
		$sql = " select
			...
			FROM t_gn_customer_db a INNER JOIN t_gn_assignment b on a.CustomerId=b.CustomerId
			LEFT JOIN t_gn_campaign c on a.CampaignId=c.CampaignId
			LEFT JOIN t_gn_policyautogen d on a.CustomerId=d.CustomerId
			LEFT JOIN t_gn_policy e on d.PolicyNumber=e.PolicyNumber
			LEFT JOIN t_lk_aprove_status f ON f.ApproveId=a.CallReasonQue
			LEFT JOIN t_gn_productplan pl ON e.ProductPlanId=pl.ProductPlanId
...
?>
	...
	/* assign navigation filter **/
		...
		var navigation = {
			custnav:'dta_qc_approval_nav.php',
			custlist:'dta_qc_approval_list.php'
		}
		...
		var defaultPanel = function(){
			if( doJava.destroy() ){
				...
				doJava.Params = { 
					...
					ProductGroup	 : datas.ProductGroup
				}
		...
		var resetSeacrh = function(){
			if( doJava.destroy() ){
				doJava.init = [
								...
								['user_id'],['start_date'],['end_date'],['ProductGroup']
							  ]
				doJava.setValue('');	
			}
		}
 ...
 /* go to call contact detail customers **/
		...
		var showPolicy = function()
		{
			...
			var arrCountRows = arrCallRows.split(','); 
				...
				{	
					...
					{
						doJava.File = 'dta_qc_detail_qa.php';
						//alert(doJava.File);
						doJava.Params = { CustomerId : arrCountRows[0],aksi:datas.aksi }
	...
	/* memanggil Jquery plug in */
		...
		$(function(){
			...
			$('#toolbars').extToolbars({
				extUrl   :'../gambar/icon',
				extTitle :[['Search'],['Search Old'],['Show Policy '],['Clear']],
				extMenu  :[['searchCustomer'],['searchDeletion'],['showPolicy'],['resetSeacrh']],
				extIcon  :[['zoom.png'],['zoom.png'],['pencil_go.png'],['cancel.png']],
				extText  :true,
				extInput :true,
				extOption:[{
						render : 4,
	...
	<!-- start : content -->
		...
		<fieldset class="corner">
			<legend class="icon-customers">&nbsp;&nbsp;Customer Approval </legend>	
