<?php
 ...
 /** get Call result Code bypass from Custid **/
	...
	function getCallBackAttempt(){
		global $db;
		
		$sql = "SELECT  a.CallAgainAttempt,a.LastCallAgainDate,a.CallReasonQue
				FROM t_gn_customer a 
				WHERE a.CustomerId='".$db->escPost('CustomerId')."'
				AND a.CallReasonQue = 2";
		$qry = $db->execute($sql,__FILE__,__LINE__);	
		if( $qry && ($rows = @mysql_fetch_assoc($qry)))
		{
				$conds['CallAgainAttempt']	= $rows['CallAgainAttempt'];
				$conds['LastCallAgainDate']	= $rows['LastCallAgainDate'];
				$conds['CallReasonQue']		= $rows['CallReasonQue'];
		}
		
		if($conds['CallAgainAttempt']>0 && $conds['LastCallAgainDate']<date("Y-m-d")){
			$sql = "Update t_gn_customer a SET a.CallAgainAttempt=0 WHERE a.CustomerId = '".$db->escPost('CustomerId')."'";
			$db -> execute($sql,__FILE__,__line__);
			$conds['CallAgainAttempt'] = 0;
		}
		return $conds;
	}
...
/** get CampaignId By CustomerId **/
	...
	$callbacklimiter = getCallBackAttempt();
	// echo "<pre>";
	// print_r($callbacklimiter);
	 // echo "</pre>";
...
<script>
	...
	var V_HANDLING_USER	 = '<?php echo $db->getSession('handling_type');?>';
...
// /* Dial to customer */
	...
	if(V_HANDLING_USER==10){
		$('#toolbars').extToolbars({
					extUrl   :'../gambar/icon',
					extTitle :[['Edit Cust Name'],['Show Detail Policy']],
					extMenu  :[['EditCustomer'],['ShowDetail']],
					extIcon  :[['folder_user.png'],['glasses.png']],
					extText  :true,
					extInput :true,
					extOption:[]
		});
		...
		$('#toolbars').extToolbars({
					extUrl   :'../gambar/icon',
					extTitle :[['Edit Cust Name'],['Show Detail Policy'],['Create Score']],
					extMenu  :[['EditCustomer'],['ShowDetail'],['newScore']],
					extIcon  :[['folder_user.png'],['glasses.png'],['application_edit.png']],
					extText  :true,
					extInput :true,
					extOption:[]
		});
	...
	var newScore = function(){
		...
		var yScreen = (Ext.DOM.window.screen.availHeight);
			doJava.winew.winconfig= { // get dojava object 
				...
				//location	: 'sys_redirect.php?page=showScoring&CustomerId='+V_DATAS_CUSTOMER,
				location	: '../coll_mon/collmon.QA.php?action=showWins&CustomerId='+V_DATAS_CUSTOMER,
				width		: xScreen,
				height		: yScreen, 
	...
	var EditPolicy = function()
	{
		try
		{
			...
			var yScreen = ($(window).height());
				...
				doJava.winew.winconfig={
						location	: V_FORM_POLICY+'?'+doJava.ArrVal(),
						width		: xScreen,
						height		: yScreen,
						windowName	: 'windowName',
						resizable	: false, 
	...
	var ShowDetail = function(){
		var xScreen = ($(window).width()-250);
		  var yScreen = ($(window).height());
			...
			doJava.winew.winconfig={
					location	: 'form.axa.qa.product.php?'+doJava.ArrVal(),
					width		: xScreen,
					height		: yScreen,
					windowName	: 'windowName',
					resizable	: false, 
 ...
 /* exit  prosess verified **/	
	...
	var exitApprove =function(){
		if(confirm('Do you want to exit from this session ?')){
			if( ClearProses() ){
				class_active.Active();
				$('#main_content').load('dta_qcrecon_nav.php');
			}
		}
		else
			return false;
		...
		<table border=0 width="70%" cellpadding="0px" cellspacing="0px">
				<tr>
					...
					<td valign="top" rowspan="3">
						<div class="box-shadow" style="border:1px solid #dddddd;width:250px;height:550px;width:'100%';padding-left:4px;">
