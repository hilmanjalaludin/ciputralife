...
/* define info customers **/
	var InitPhp	     = '../class/tpl.wa.contact.detail.php?';
	var phoneCall    = {
						initPhone  : false,
						initType   : '', 
						initNumber : '',
...
/* valid Form **/
 ...
 /* function sava call  **/
	...
	var saveCallAction = function()
	{
		...
		var WAResult = doJava.dom('wa_result').value;
		// alert("WAResult-"+WAResult);
		var CallRemarks = doJava.dom('call_remarks').value;
		// alert("CallRemarks-"+CallRemarks);
		//return false;
		...
		if( WAResult!='' ){
				/* if(V_VERIFIED != ''){
					...
					{ */
						doJava.File = '../class/class.save.activitycall.wa.php';
						doJava.Params = {
							action:'save_activity_call',
							campaignid : CampaignId,
							customerid : CustomerId,
...
/* cancel activity **/
	...
	var CancelActivity = function(){
		...
		else
		{
			class_active.Active();
			if( V_VERIFIED!='' ){
				$('#main_content').load('src_customer_fu_wa_nav.php');
			}
			else{
		...
		if(!phoneCall.initSave){
			...
			new(function(){
				...
				if( Customers.valid )
				{
					...
					doJava.File = "dta_wa_contact_detail.php";
					doJava.Params = { 
...
/* save activity **/
	...
	var saveActivity = function()
	{
		...
		var WAResult = parseInt(doJava.dom('wa_result').value);
...
**/
	...
	var getPhoneNumber = function(phoneType){
		if( phoneType!='')
		{
			doJava.File = "../class/tpl.wa.contact.detail.php"
			doJava.Params = {
		...
		<tr>
			<td  width="80%" valign="top">
				...
				<div class="box-shadow">
					<table width="99%" style="margin-bottom:8px;"  align="center">
						<tr>
							<td id="recording_file" width="50%" valign="top" style="background-color:#FFFFFF;"></td>
							<td id="recording_play" width="50%" valign="top" style="background-color:#FFFFFF;"></td>
						</tr>	
					</table>
				</div>
