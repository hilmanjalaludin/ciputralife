...
/* action get Open toolbar **/	
	...
	var tna = function(){
        ...
        var CallResult = doJava.dom('call_result').value;
        var FuType     = TNA;
        if( CallResult != '' && CallResult  == FOLLOW_UP_FA) {
			var xScreen = ($(window).width());
			var yScreen = ($(window).height());
			doJava.Params ={
				CampaignId 	: CampaignId,
			...
			doJava.winew.winconfig={
				location 	: 'frm.followup.fa.php?'+doJava.ArrVal(),
				width 		: xScreen,
				height 		: yScreen,
				windowName 	: 'windowReferal',
				resizable 	: false, 
	...
	var saveActivity = function()
	{
		...
		if (doJava.winew.winHwnd.closed==undefined) {
			if( isCreateFormFa() ){
				...
				alert('Info , Please Create the form follow up fa !'); return false;
			}
		} else {
			if( !doJava.winew.winHwnd.closed) { 
				alert('Please close your Create form follow up fa window..!'); return false;
			} else {
				if(phoneCall.initCreate)
				{
					if( isCreateFormFa())
