...
/** get status closing ****/
 ...
 /** set  genral query SQL  **/
	...
	$sql = "SELECT 
				...
				p.Premi* IF(g.PayModeId=2,12,1) APE,
				c.CampaignNumber,
				c.CampaignName  
			FROM t_gn_customer a INNER JOIN t_gn_assignment b on a.CustomerId=b.CustomerId 
				...
				LEFT JOIN t_gn_insured i ON a.CustomerId=i.CustomerId
				LEFT JOIN t_gn_policy p ON p.PolicyId = i.PolicyId
				LEFT JOIN t_gn_productplan g ON p.ProductPlanId = g.ProductPlanId";
 ...
 /** create set filter SQL if found **/	
	...
	$filter =  " AND b.AssignAdmin is not null 
				 ...
				 and (f.CallReasonId is not null or f.CallReasonCategoryId is not null)
				 #AND f.CallReasonId NOT IN (". getWAEmailStatus(). ")
				 AND b.AssignBlock=0 
	...
	if( $db -> havepost('call_status'))
		$filter.=" AND a.CallReasonId = '".$db->escPost('call_status')."'"; 
	
	if( $db->havepost('start_date') && $db->havepost('end_date')) 
		$filter .= " AND a.CustomerUpdatedTs >= '".$db->formatDateEng($_REQUEST['start_date'])." 00:00:00' 
					 AND a.CustomerUpdatedTs <= '".$db->formatDateEng($_REQUEST['end_date'])." 23:00:00' "; 
...
<thead>
	<tr height="20"> 
		...
		<th nowrap class="custom-grid th-middle" align='left'>&nbsp;<span class="header_order" onclick="extendsJQuery.orderBy('a.CustomerFirstName');"><b style='color:#608ba9;'>APE</b></span></th>
...
<tbody>
	...
	?>
			<tr class="onselect">
				...
				<td class="content-middle" width='40%' nowrap style="color:green;font-weight:bold;"><?php echo $row -> APE; ?></td>
