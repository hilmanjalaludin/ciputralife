	...
	*/
	...
	/* function set style css **/
		...
		function ValidationCheck()
		{
			...
			{
				$result = array( 'result' => $rows['QaProsess'] );
			}
		...
		function getUserList($cond='')
		{
			...
			else if( $this->getSession('handling_type')==8){
				$sql = "Select a.* from tms_agent a inner join cc_agent b on a.id=b.userid  where a.user_state =1 
				and a.handling_type='4'";
			}
			...
			else if( $this->getSession('handling_type')==11){
				$sql = "Select a.* from tms_agent a inner join cc_agent b on a.id=b.userid  where a.user_state =1 
				and a.handling_type='4'";
			}
		...
		function getUserSPV($cond='')
		{
			$sql = "Select a.* from tms_agent a where a.user_state =1 and a.handling_type=3 ";
			if( $sql ){
				$query = $this ->execute($sql,__FILE__,__LINE__);
				while( $row = $this->fetchrow($query))
				{
		...
		function getUserAM($cond='')
		{
			$sql = "Select a.* from tms_agent a where a.user_state =1 and a.handling_type=2";
		...
		private function getProductType()
		{
			...
			foreach( $qry -> result_assoc() as $rows ){
				$data[$rows['product_category_id']] = $rows['product_category_code'];
			}
		...
		private function getCampaignAssigment($cond='')
		{
			...
			//where date(if(a.CampaignExtendedDate is null, a.CampaignEndDate,a.CampaignExtendedDate)) >= date(now())
		...
		private function getResultStatus($cond='')
		{
			$sql = "select a.CallReasonId, a.CallReasonCode, a.CallReasonDesc from t_lk_callreason a 
					where a.CallReasonStatusFlag=1 and a.CallReasonEvent=1
			...
			<div id="result_content_add" class="box-shadow" style="padding-bottom:4px;margin-top:2px;margin-bottom:8px;">
				<table cellpadding="3px;">
					<tr>
						...
						<td>
							<select name="campaign_id" id="campaign_id" class="select" style="width:200px;">
					...
					<tr>
						...
						<td class="text_header"> Campaign Status</td>
						<td>
							<select name="cmp_status" id="cmp_status" class="select" style="width:200px;">
								<option value=""> -- Choose --</option>
								<?php  
									foreach (array('0'=>'Not Active','1'=>'Active') as $val => $text)
									{
										
											if( (STRING)$val==(STRING)$this->escPost('cmp_status') ){
					...
					<tr>
						...
						<td>
							<select name="call_result" id="call_result" class="select" style="width:200px;">
								<option value=""> -- Choose --</option>
					...
					<tr>
						...
						<td>
							<select name="user_id" id="user_id" class="select" style="width:200px;">
								<option value=""> -- Choose --</option>
								<?php $this->getUserList($this->escPost('user_id')); ?>
							</select>
						</td>
						
						<td class="text_header"> User SPV </td>
						<td>
							<select name="user_spv" id="user_spv" class="select" style="width:200px;">
								<option value=""> -- Choose --</option>
								<?php $this->getUserSPV($this->escPost('user_spv')); ?>
							</select>
						</td>
						
						<td class="text_header"> User AM </td>
						<td>
							<select name="user_am" id="user_am" class="select" style="width:200px;">
								<option value=""> -- Choose --</option>
								<?php $this->getUserAM($this->escPost('user_am')); ?>
							</select>
						</td>
					</tr>
					<tr>
						<td class="text_header"> Product Group </td>
						<td><?php $this -> DBForm -> jpCombo('ProductGroup','select long',$this->getProductType(),$this->escPost('ProductGroup') ); ?></td>
					</tr>
