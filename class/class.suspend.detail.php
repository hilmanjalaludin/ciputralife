	...
	**/
	...
	/** stop prosess on QC **/
		...
		function resetProses(){
			$sql = " Update t_gn_customer a SET a.QaProsess=0 WHERE a.CustomerId='".$this->escPost('customerid')."'";
	 ...
	 ** ***********************************************/	
		...
		?>
			<script>
				var getprimaryPhone = function(){
					$(function(){
					doJava.Params = { 
						action:'get_primary_phone',
						customerid:V_DATAS_CUSTOMER
					}
					$('#phone_primary_number').load( V_DATAS_INITCLAS+'?'+doJava.ArrVal() );
				});
				}
		...
		?>	
				<select class="txt_input combo" id="call_primary_phone" onchange="setCallNumber(this.value);">
					<option value=""> -- Choose --</option>
			<?php 
 ...
 ** ************************************************/	
	...
	/** function result qa ***/
		...
		//untuk report scoring (DC QC)
		private function updateapprovalscore()
		{
			$this -> set_mysql_update('t_gn_approval_score_qa', 
							array(
								'approval_id_update' => $_REQUEST['status'],
								'approval_qa_id_update' => $_SESSION['UserId'],
