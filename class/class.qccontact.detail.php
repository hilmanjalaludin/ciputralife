<?php
	...
	**/
	...
	/** *************************/	
		...
		function __construct(){
			parent::__construct();
				...
				$this -> FuType 	= $this -> escPost('FuType');
		}
	...
	/** *************************/	
		...
		{	
			if( $this->havepost('action') )
			{
				switch($this -> Action )
				{
					case 'form'   			 : $this -> TypeForm(); 			break;
					case 'history_contact'   : $this -> HistoryContactTpl(); 	break;
					case 'history_contacts'  : $this -> HistoryContactTpls(); 	break;
					...
					case 'save_qcs'			 : $this -> saveVerifieds(); 		break;
					...
					case 'check_status_fa'	 : $this -> CheckStatusQcFa(); 		break;
					...
					case 'get_callbackcount' : $this -> getCallBackCount($this -> escPost('customerid'));	break;
				}
			}
		}
		...
		function CheckStatusQcFa()
		{
			$_conds = array('result' => 0);
			
			$sql = "select a.FuQAStatus from t_gn_followup a where a.FuCustId = '".$this->escPost('customerid')."'";
			$qry = $this->query($sql);
	...
	/** save remider if pending **/
		...
		function getCallBackCount($CustomerId = Null){
			if($CustomerId != Null){
				$sql = "Select CallAgainAttempt from t_gn_customer where CustomerId = ".$CustomerId;
				$qry = $this -> query($sql);
				if( $qry -> result_num_rows() > 0 ){
					echo $qry -> result_singgle_value();
				}else
					echo 0;
				}
		}
 ...
 ** ************************************************/	
	...
	function checkAllInsured()
	{
		...
		function saveVerified()
		{
			...
			if($id != "") {
				$sql = " UPDATE t_gn_customer a 
						 JOIN t_gn_followup b
						SET 
							a.QueueId='$userid', 
							a.CallReasonQue='$status',
							a.CustomerRejectedDate=NOW(),
							b.FuQAId = '$userid',
							b.FuQAStatus = '$status',
							b.FuUpdateTs = NOW()
						WHERE a.CustomerId ='$customerid' AND b.FuId = '$id'
					";
			} else {
		...
		function getType () {
			$data = array();
			$sql = "SELECT * FROM t_gn_followup tgf WHERE FuCustId = '".$this -> escPost('customerid')."' AND FuType = 1";
			// echo $sql;
			$qry = $this -> query($sql);
			foreach($qry -> result_assoc() as $row) {
				$data['type'] = $row['FuType'];
			}
			
			return $data;
		}
		
		function saveVerifieds()
		{
			$data 		= $this -> getDataStatus();
            $datas  	= $this -> getType();
			$result		= array('result'=>0);
	...
	/** function result qa ***/
		...
		function ReasonOnQAS()
		{
			$datas = $this-> getDataStatus();
			$sql ="select a.AproveName from t_lk_aprove_status a where a.ApproveId='".$datas['status']."'";
			// echo $sql;
			$qry = $this -> query($sql);
			if( !$qry -> EOF() )
			{
	 ...
	 ** ************************************************/
		...
		private function HistoryCalls()
		{
			$data 	= $this -> getDataStatus();
			$status = $this -> escPost('status');
			$codec  = $this -> escPost('codec');
			$notes 	= $this -> escPost('notes');
			$notes .= " ( {$this -> ReasonOnQAS()} ) ";
			$CallHistory = array(
				'CustomerId'			=> $this -> escPost('customerid'), 
				'CallReasonId'			=> $this -> getCallHistoryReason(),
				'CreatedById'			=> $this -> getSession('UserId'), 
	...
	/** *************************/	
		...
		function getDatass() {
			...
			$sql = 
				"
					...
					JOIN t_lk_aprove_status b ON a.FuQAStatus = b.ApproveId
					WHERE a.FuCustId = '".$this -> CustomerId."'
				";
			// echo $sql;
			$qry = $this -> query($sql);

			foreach( $qry -> result_assoc() AS $rows ) {
				$data['status'] = $rows['AproveName'];
				$data['id']     = $rows['FuCustId'];
			}
		...
		function getDataStatus() {
			$data = array();
			$sql = "
					SELECT a.*,b.* FROM t_gn_followup a 
					LEFT JOIN t_lk_aprove_status b ON a.FuQAStatus = b.ApproveId
					WHERE a.FuCustId = '".$this -> CustomerId."' and a.FuId = '".$this -> escPost('FuId')."'";
			//echo $sql;
			$qry = $this -> query($sql);
			foreach($qry -> result_assoc() as $rows) {
				$data['status'] 	= $rows['FuQAStatus'];
				$data['id_status']  = $rows['ApproveId'];
				$data['id']  	    = $rows['FuId'];
				$data['cus_id']	    = $rows['FuCustId'];
				$data['code']	 	= $rows['AproveCode'];
		...
		*/
		function getData()
		{
		    ...
		    $sql = "select
		    		...
		    		where a.FuCustId = '{$this -> CustomerId}' AND a.Futype = 1";
		    // echo $sql;
		    $qry = $this-> query($sql);
		...
		function getDataMtf()
		{
		    ...
		    $sql = "select
		    		...
		    		from t_gn_followup a 
		    		inner join t_gn_customer b on a.FuCustId = b.CustomerId 
		    		where a.FuCustId = '{$this -> CustomerId}' AND a.Futype = 3";
		    // echo $sql;
		    $qry = $this-> query($sql);
					...
					<div style="height:220px;overflow:auto;border:1px solid #dddddd;">
						...
						//var_dump($data);
							$sql = " SELECT 
										...
										a.*, 
										c.*, 
										date_format(a.CallHistoryUpdatedTs,'%d-%m-%Y %H:%i') as Calldate
									FROM t_gn_callhistory a 
									LEFT JOIN tms_agent b ON a.UpdatedById= b.UserId
									LEFT JOIN t_lk_callreason c ON a.CallReasonId=c.CallReasonId
									LEFT JOIN t_gn_followup d ON a.FuId=d.FuId
									
									";

								if( $datas['status'] == 0) {
									$sql .= "WHERE a.CustomerId ='".$this -> CustomerId."' AND a.FuId = '".$this -> escPost('FuId')."' AND  a.TypeFa = '".$this-> escPost('FuType')."' ORDER BY a.CallHistoryUpdatedTs DESC";
								} else {
									$sql .= "WHERE a.CustomerId ='".$this -> CustomerId."'AND a.FuId = '".$this -> escPost('FuId')."' AND  a.TypeFa = '".$this-> escPost('FuType')."' ORDER BY a.CallHistoryUpdatedTs DESC";
								}
						...
						<?php
							$sql = " SELECT 
									...
									FROM t_gn_callhistory a left join tms_agent b on a.UpdatedById= b.UserId
										left join t_lk_callreason c on a.CallReasonId=c.CallReasonId
									WHERE a.CustomerId ='".$this -> CustomerId."' AND a.FuId IS NULL AND a.TypeFa IS NULL	
