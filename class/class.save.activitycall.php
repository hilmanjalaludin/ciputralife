...
class CallActivity extends mysql
{
	...
	/** index data **/
		...
		function index()
		{
			switch( $this -> Action )
			{
				...
				case 'isValidformss'		:  $this -> isValidforms(); 		break;
 ...
 /** save Activity Call **/
		...
		{
			//first get data
			$data 	= $this -> getDataTnaNmtf();
			$datas 	= $this -> getDataInterest();
		...
		/**
		* function for update flag is form 
		* author : didi ganteng;
		*/
		function update_isform() {
			$sql = 
			"
				UPDATE 
					t_gn_customer 
		...
		*/
		// function getDataFa() {
			...
			// foreach($qry -> result_assoc() as $rows) {
				...
				// );
			// }
			// DEFAULT save FuId and type fa sementara di remarks dlu
			// return $data;

		// }
		
		/*
		* first get data form mdtf and tnas 
		* author : didi ganteng
		* 2018-04-13
		*/
		function getDataTnaNmtf() {
			$datas = array();
			$sql = 
				"
					SELECT 
						tgf.* , 
						count(tgf.FuCustId) as total 
					FROM t_gn_followup tgf 
					WHERE tgf.FuCustId = '".$this -> CustomerId."' AND tgf.FuType IN(2,3) 
					GROUP BY tgf.FuId
				";
			$qry = $this->query($sql);
			
			foreach($qry-> result_assoc() as $record){
				
				$datas['type']	= $record['FuType'];
				$datas['id']	= $record['FuId'];
			}	
			return $datas;
		}
		...
		// function getFa() {
			...
			// $SQL = "select a.* from t_gn_followup a where a.FuCustId='".$this->CustomerId."' and a.FuType IN (1)";
			// $qry = $this->query($SQL);
		...
		/**
		* function first get data interest
		* author : didi ganteng
		**/
		...
		function saveHistoryCall()
		{
			...
			{
				$this -> gethistoryid = $this->get_insert_id();
				//jika ada pengisian form di tnas atau mdtf maka 2 kali save call history;
				//untuk tracking contact history di QA
				if($datas['id'] != "") {
					$this -> _save_history_call();
				}
				return true;
			}
			else
		...
		/**
		* private function for save history call , REMARK tnas and mdtf
		* author : didi ganteng
		**/
		private function _save_history_call() {
			//first  get data 
			$datas = $this -> getDataTnaNmtf();
			...
			$queryHistory = $this -> set_mysql_insert("t_gn_callhistory",$InsertData);
