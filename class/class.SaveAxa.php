...
private function _set_save_insured( $Insured = null )
{
	...
	// $this -> set_mysql_insert('t_lk_city', array(
		// 'CItyNames' => strtoupper($this -> escPost('InsuredPOB'))
	// ));
}
/*
 * @ def 		: AXA_Save / constructor 
...
public function index()
{
	...
	switch( $this -> escPost('action')) 
	{
		...
		case '_get_age_holder'	: self::_get_holder_age(); 		break;
		...
		case '_update_cc'		 : self::_update_cc();  		break;
		case '_insert_log_cc'	 : self::_insert_log_cc();  	break;
		...
		case 'show_card_numberccs' : self::show_card_numberccs(); break;
        case 'show_card_numbersaving' : self::show_card_numbersaving(); break;
        case '_load_ivr_bank' : self::load_ivr_bank(); break;
        case 'payment_checking' : self::payment_checking(); break;
        // case '_check_digit' : self::_check_digit(); break;
		// case 'move_cc' : self::move_cc();break;
		// case 'move_saving' : self::move_saving();break;
		case 'suggest_city' : self::_get_json_city(); break;
	}
}


private function payment_checking() // payment_checking
{
	$_conds = array('result' => 0 );
	$custid = $this -> escPost('CustomerId');
	$sql = sprintf("SELECT ( 
			(SELECT count(a.AccountNo) b
			FROM t_gn_customer_bank_account a
			WHERE a.CustomerId = %s )
			+
			(SELECT count(a.CreditCardNo) c
			FROM t_gn_customer_cc  a
			WHERE a.CustomerId = %s )
			) num  ", $custid,$custid);
	...
	{
		$_conds = array('result' => $rows["num"]);
	}
	echo json_encode($_conds);
	
}

private function _get_json_city()
{
	$city = array();
	$term = $this->escPost('term');
	if(!is_null($term) && strlen($term) >= 3)
	{
		$sql = "SELECT a.CityId,a.CItyNames FROM t_lk_city a WHERE a.CityFShowFlag=1 AND a.CItyNames like '%".strtoupper($term)."%'" ;
		$qry = $this -> query($sql);
		foreach( $qry -> result_assoc() as $rows ){
			$city[$rows['CityId']] = $rows['CItyNames'];
		}
	}
...
private function load_ivr_bank()
{
	...
	if($this->havepost('IvrPayMethode') and ($this->escPost('IvrPayMethode') != "undefined") )
	{
		...
		if($IvrPayMethode == 1){
			...
			{
				$this -> DBForm -> jpCombo("IvrBankId", 'select long',$this -> Customer -> Bank());
			}
		}else if($IvrPayMethode == 2){
			$bank_list = $this->getBankByIvrPay( $IvrPayMethode );
			if(count( $bank_list ) > 0)
			{
				$this -> DBForm -> jpCombo("IvrBankId", 'select long',$bank_list);
			...
			{
				$this -> DBForm -> jpCombo("IvrBankId", 'select long',$this -> Customer -> SavingBank());
			}
		}
	}
	else
...
private function get_card_numbercc()
{
	...
	$sql = "select a.CreditCardId,a.CreditCardNo,a.ExpireDate, if(fx.ValidCCPrefix=left(a.CreditCardNo, 6),1,0) isValid,
				DATE_FORMAT(a.CreatedOn,'%d-%m-%Y %H:%i:%s') as CreatedOn,
				a.CustomerId
				from t_gn_customer_cc a
				left join t_lk_validccprefix fx ON left(a.CreditCardNo, 6) = fx.ValidCCPrefix
				where 1=1";
	...
	foreach($qry -> result_assoc() as $rows ) 
	{
		...
		$digit_ivr[$rows["CreditCardId"]]["cust_id"] = $rows["CustomerId"];
		...
		$digit_ivr[$rows["CreditCardId"]]["isValid"] = $rows["isValid"];
	}
	return $digit_ivr;
}
...
function show_card_numbercc(){
	...
	if($this -> havepost('CustomerId'))
	{
		...
		{
			 $this-> _insert_log_cc($array_value['cust_id'], $array_value['card_number']);
			//$result = str_replace('{"result":1,"log_id":3}', " ", $result);
			$len_digit = strlen($array_value['card_number']);
			// <td class=\"rows-first\" align=\"left\">".$array_value['isValid']==1?"CreditCard is Valid":"CreditCard is not Valid!!!"."</td>
			echo "<tr>
				...
				if( $this->getSession("handling_type") == 5) {
					echo "<td class=\"rows-first\" align=\"left\">".$array_value['card_number']."</td>";
				} else if($this->getSession("handling_type") == 10) {
					echo "<td class=\"rows-first\" align=\"left\">".$array_value['card_number']."</td>";
				} else {
					echo "<td class=\"rows-first\" align=\"left\">".substr_replace($array_value['card_number'], 'xxxxxxxxxxxx',4,12)."</td>";
				}
			echo   "<td class=\"rows-first\" align=\"left\">".$array_value['CreatedOn']."</td>
...
private function _get_data_survey($product=0,$questioner_type=0)
{
	...
	if($questioner_type!=0)
	{
		$sql = "SELECT a.prod_survey_id,c.survey_quest_id,c.survey_question, d.ans_label,e.type_survey,quest.questioner_type
			...
			INNER JOIN t_lk_type_ans_survey d ON b.type_ans_id = d.type_ans_id
			INNER JOIN t_lk_type_survey e ON d.type_survey_id=e.type_survey_id
			WHERE 1=1 and quest.product_id = ".$product."
			AND quest.questioner_flag = 1
...
function _get_valid_prefix()
{
	...
	if(strlen($this->escPost('card_num')) == 16)
	{
...
function _update_qc()
{
	...
	*/
	function _update_cc() {
		...
		$result = array("result" => 0, "message" => false);
		$CustomerId =  $this -> escPost('CustomerId');

		$update = array(
			"CreditCardNo" => $this->escPost('CreditCardNo')
		);
		$kondisi = array("CustomerId" => $this -> escPost('CustomerId'));

		//$sql = "UPDATE t_gn_customer_cc tgc SET tgc.CreditCardNo = '".$this->escPost('CreditCardNo')."' WHERE tgc.CustomerId = '".$CustomerId."'";
		//echo $sql;
		//$query = $this->execute($sql);

		if( $this->set_mysql_update("t_gn_customer_cc",$update, $kondisi) )
		{
			$this->_update_log_cc($CustomerId, $this->escPost('CreditCardNo'));
			$result = array('result' => 1, "message" => true);
			//return true;
		}
	...
	/**
	* function first get data customer cc card
	* from t_gn_customer_cc
	* param : customer_id
	* author : didi ganteng
	*/
	// function _first_get_data_cc() {
	...
	// 				tgcc.CreditCardNo
	// 			FROM 
	// 				t_gn_customer_cc tgcc 
	// 			WHERE 
	// 				tgcc.CustomerId = '".$this -> havepost('CustomerId')."'
	// 		";
	// 	///echo $sql;
	// 	$query = $this -> query($sql);

	// 	foreach($query -> result_assoc() as $row) {
	// 		$data['cust_id']  = $row['CustomerId'];
	// 		$data['no_card']  = $row['CreditCardNo'];
	// 	}
	...
	/**
	* function for insert log customer cc number
	* parameter cust id  is null and cc card is null
	* author : didi ganteng
	*/
	private function _insert_log_cc($cust_id = null, $cc_card= null) {

		$table = "t_gn_customer_cc_log";
		$datas = $this -> first_get_data($cust_id);

		$CustomerId 	= $this-> escPost("CustomerId");
		$cc_card_old 	= $this -> escPost("customer_cc_card_old");
		$cc_card_new 	= $this-> escPost("customer_cc_card_new");
		$updated_by_id 	= $this->getSession('UserId');
		$created_date  	= date("Y-m-d H:i:s");

			$save_to_db = array(
				"log_customer_id" 		=> $cust_id,
				"customer_cc_card_old" 	=> $cc_card,
				"log_cc_updated_by_id" 	=> $updated_by_id,
				"log_created_date"     	=> $created_date
			);

		if($datas['ID'] != "") {

			$sql = "UPDATE t_gn_customer_cc_log tgcc SET 
						tgcc.log_customer_id = '$cust_id',
						tgcc.customer_cc_card_old = '$cc_card',
						tgcc.log_cc_updated_by_id = '$updated_by_id',
						tgcc.log_created_date = '$created_date'
					WHERE tgcc.log_id = '".$datas['ID']."'
					";
			...
			$query = $this -> set_mysql_insert($table,$save_to_db);
		}
	...
	* param : cust_id
	*/
	function first_get_data($cust_id = null) {
		$data = array();
		$sql =  "SELECT * FROM t_gn_customer_cc_log tgccl WHERE tgccl.log_customer_id = '".$cust_id."'";
	...
	/**
	* function for update log customer cc number
	* parameter cust id  is null and cc card is null
	* author : didi ganteng
	*/
	function _update_log_cc($cust_id = null, $cc_card = null) {
		...
		$update_to_db  = array(
			"customer_cc_card_new"	=> $cc_card,
			"log_cc_updated_by_id"	=> $updated_by_id,
			"log_updated_date"		=> $updated
		);
		$kondisi = array("log_customer_id" => $cust_id);

		// $SQL = "
		// 		UPDATE t_gn_customer_cc_log tgcc SET 
		// 			tgcc.customer_cc_card_new = '$cc_card',
		// 			tgcc.log_cc_updated_by_id = '$updated_by_id',
		// 			tgcc.log_updated_date	 = '$updated'
		// 		WHERE tgcc.log_customer_id = '$cust_id'
		// 		";
		// echo $SQL;

		$query = $this->set_mysql_update($table, $update_to_db, $kondisi); //$this -> execute($SQL,__FILE__,__LINE__)
		if( $query ) {
			return true;
		} else {
			return false;
...
private function _get_personal_premi()
{
 ...
 if( class_exists('Customer'))
 {
		...
		$_argument['start_age']	  = ( $this -> havepost('PersonalAge')?(float)$this -> escPost('PersonalAge'):false );
...
function _get_Detail()
{
	...
	if( count($rows) > 0 )
	{
		...
		$_data['InsuredPOB'] = $rows['InsuredPOB'];
 ...
 */
 ...
 private function _set_save_data_payer()
 {
	...
	if( $this -> set_mysql_insert('t_gn_payer', array(
		...
		'PayerLastName' => $this -> escPost('PayerLastName'),
		'GenderId' => $this -> escPost('PayerGenderId'),
		'PayerPOB' => $this -> escPost('PayerPOB'),
		'PayerDOB' => $this -> formatDateEng($this -> escPost('PayerDOB')),
		'PayerAddrType' => $this -> escPost('PayerAddrType'),
		'PayerAddressLine1' => $this -> escPost('PayerAddressLine1'),
		'IdentificationTypeId' => $this -> escPost('PayerIdentificationTypeId'),
		...
		'PayerCity' => $this -> escPost('PayerCity'),
		'PayerAddressLine2' => $this -> escPost('PayerAddressLine2'),
		'PayerMaritalStatus' => $this -> escPost('PayerMaritalStatus'),
		'certificat_type' => $this -> escPost('PayerCertificateStatus'),
		'PayerHomePhoneNum' => $this -> escPost('PayerHomePhoneNum'),
		'PremiumGroupId' => ($this -> escPost('pyisholder')?$this -> escPost('HoldGroup'):4),
		'PayerZipCode' => $this -> escPost('PayerZipCode'),
		'PayerAddressLine3' => $this -> escPost('PayerAddressLine3'),
	...
	// $this -> set_mysql_insert('t_lk_city', array(
		// 'CItyNames' => strtoupper($this -> escPost('PayerPOB'))
	// ));
	return $_conds;
 }
/*
 ...
 */
 ...
 function get_size_insured($PolicyNumber)
 {
	...
	$sql = "SELECT b.InsuredId, e.PremiumGroupDesc,a.Premi,
			...
			c.ProductPlanPremium, b.InsuredFirstName, b.InsuredPOB, b.InsuredDOB, b.InsuredAge, b.PremiumGroupId,
	...
	foreach($qry -> result_assoc() as $rows ) 
	{
		...
		$_conds[$i]['InsuredPOB']= $rows['InsuredPOB'];
...
{ 
	...
	{
		echo "  <tr>
					<td class=\"rows-first\" style=\"background-color:#FFFCCC;height:24px;padding-left:4px;font-weight:bold;color:green;font-size:12px;\" align=\"left\" colspan=\"11\">". $rows['PolicyNumber']. "  / " . $rows['NamaPemegangPolicy'] ." </td>
				</tr>";
			...
			/** calculation evry Number policy **/
				...
				foreach( self::get_size_insured($rows['PolicyNumber']) as $rows )
				{
					...
					echo "  <tr>
								...
								<td class=\"rows-first\" align=\"left\">".strtoupper($rows['InsuredPOB'])."</td>
		...
		echo "  <tr>
					...
					<td class=\"rows-first\" style=\"border-top:1px solid red;\" align=\"left\" colspan=\"5\"></td>
	...
	echo " <tr>
				...
				<td class=\"header-first\" style=\"border-top:1px solid red;\" align=\"left\" colspan=\"5\">&nbsp;</td>
...
private function _set_save_data_holder(){
	$_conds = false;
	$payment_cc = "1";
	$payment_saving = "2";
	$paytypeid = NULL;
	...
	if( $this -> set_mysql_insert('t_gn_holder', array(
		'CustomerId' => $this -> escPost('CustomerId'),
		'HolderFirstName' => $this -> escPost('HolderFirstName'),
		'HolderLastName' => $this -> escPost('HolderLastName'),
		'HolderGenderId' => $this -> escPost('HolderGenderId'),
		'HolderPOB' => $this -> escPost('HolderPOB'),
		'HolderDOB' => formatDateEng( $this -> escPost('HolderDOB') ),
		'HolderPosition' => $this -> escPost('HolderPosition'),
		'HolderOccupation' => $this -> escPost('HolderOccupation'),
		'HolderIncome' => $this -> escPost('HolderIncome'),
		'HolderCompany' => $this -> escPost('HolderCompany'),
		'HolderMobilePhoneNum' => $this -> escPost('HolderMobilePhoneNum'),
		'HolderMaritalStatus' => $this -> escPost('HolderMaritalStatus'),
		'HolderIdentificationTypeId' => $this -> escPost('HolderIdentificationTypeId'),
		'HolderIdentificationNum' => $this -> escPost('HolderIdentificationNum'),
		'HolderRelationshipTypeId' => $this -> escPost('HolderRelationshipTypeId'),
		'HolderAddrType' => $this -> escPost('HolderAddrType'),
		'HolderAddressLine1' => $this -> escPost('HolderAddressLine1'),
		'HolderAddressLine2' => $this -> escPost('HolderAddressLine2'),
		'HolderProvinceId' => $this -> escPost('HolderProvinceId'),
		'HolderCity' => $this -> escPost('HolderCity'),
		'HolderEmail' => $this -> escPost('HolderEmail'),
		'HoldersBankId' => $this -> escPost('HoldersBankId'),
		'HolderBankBranch' => $this -> escPost('HolderBankBranch'),
		'HolderCreditCardNum' => $this -> escPost('HolderCreditCardNum'),
		'HolderOfficePhoneNum' => $this -> escPost('HolderOfficePhoneNum'),
		'HolderCreditCardTypeId' => $this -> escPost('HolderCreditCardTypeId'),
		'HolderZipCode' => $this -> escPost('HolderZipCode'),
		'HolderCreatedTs' => date('Y-m-d H:i:s'),
		'HolderCreatedById' => $this -> getSession('UserId')
		...
		if( $this -> set_mysql_update('t_gn_holder', 
		array(
		'HolderFirstName' => $this -> escPost('HolderFirstName'),
		'HolderLastName' => $this -> escPost('HolderLastName'),
		'HolderGenderId' => $this -> escPost('HolderGenderId'),
		...
		'HolderAddressLine1' => $this -> escPost('HolderAddressLine1'),
		'HolderAddressLine2' => $this -> escPost('HolderAddressLine2'),
		'HolderAddressLine3' => $this -> escPost('HolderAddressLine3'),
		'HolderAddressLine4' => $this -> escPost('HolderAddressLine4'),
		'HolderProvinceId' => $this -> escPost('HolderProvinceId'),
		'HolderCity' => $this -> escPost('HolderCity'),
		'HolderEmail' => $this -> escPost('HolderEmail'),
		'HoldersBankId' => $this -> escPost('HoldersBankId'),
		...
		'HolderCreditCardTypeId' => $this -> escPost('HolderCreditCardTypeId'),
		'HolderZipCode' => $this -> escPost('HolderZipCode'),
		'HolderUpdatedTs' => date('Y-m-d H:i:s'),
		'HolderUpdatedById' => $this -> getSession('UserId'),
		),array('CustomerId'=>$this -> escPost('CustomerId')))){
			$_conds = true;
		}
	}
