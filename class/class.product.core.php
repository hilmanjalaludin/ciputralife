...
function SaveProductCore()
{
	...
	$statusgender = $this->escPost('StatusGender');
						...
						foreach( $GroupPremi as $_KeyGroupId => $VGroupId )
						{
							...
							for( $start=0; $start<$_count; $start++)
							{
								for( $plan =1; (INT)$plan<=$_sizes; $plan++ ) 
								{
									...
									$ProductPlanPreminumMale = $this->escPost('plan_premiMale_'. $VPayMode .'_'. $VGroupId .'_'. $_postz .'_'. $plan);
									$ProductPlanPreminumFemale = $this->escPost('plan_premiFemale_'. $VPayMode .'_'. $VGroupId .'_'. $_postz .'_'. $plan);
								/** paymode can't empty ***/
									...
									if( ($PayModeId!='') && ($PayModeId!=0) ) 
									{
										...
										$_SQL['ProductPlanAgeEnd'] = $ProductPlanAgeEnd;
                                                                                if($statusgender == 1){
                                                                                    $_SQL['ProductPlanGender'] = TRUE;
                                                                                    $_SQL['ProductPlanPremiumMale'] = $ProductPlanPreminumMale;
                                                                                    $_SQL['ProductPlanPremiumFemale'] = $ProductPlanPreminumFemale;
                                                                                }else{
...
function getContents()
{
	...
	$GroupPremi = self::getPremiumGroup();
        $gender = $this->havepost('Statusgender');
        echo 'status gender : '.  $this->_Gender;
 ?>
			...
			<div>
				...
				<legend class="icon-customers" style="color:red;font-weight:bold;">&nbsp;&nbsp;&nbsp;<?php echo  $GroupPremi[$_GroupValue]; ?></legend>
					<table cellpadding="4px">
						<tr>
                                                    <?php if($this->_Gender == 1) { ?>
                                                    <td rowspan="2" style="background-color:#0099FF;color:#FFFFFF;"> AGE BAND </td>
                                                    <?php }else { ?>
							...
							<?php for( $header=1; $header <= $this -> _ProductPlan; $header++) { ?>                                                    
                                                        <?php if($this->_Gender == 1){ ?>
                                                            <td colspan="2" style="font-weight:bold;background-color:#0099FF;color:#FFFFFF;">PLAN <?php echo $header; ?></td>
                                                        <?php } else { ?>
								<th style="font-weight:bold;background-color:#0099FF;color:#FFFFFF;">PLAN <?php echo $header; ?></th>                                                        
                                                ...
                                                <tr>
                                                    <?php for( $header=1; $header <= $this -> _ProductPlan; $header++) { ?>
                                                    <td style="font-weight:bold;background-color:#0099FF;color:#FFFFFF;">Male</td>
                                                    <td style="font-weight:bold;background-color:#0099FF;color:#FFFFFF;">Female</td>
                                                    <?php }?>
                                                </tr>
                                                <?php } ?>
						...
						<tr>
							...
							<?php for( $plan =1; $plan <= $this -> _ProductPlan; $plan++){ ?> <!-- start counter of plan  --> 
                                                        <?php if($this->_Gender == 1){ ?>
                                                        <td><?php $this->DBForm->jpInput("plan_premiMale_{$PayModeValue}_{$_GroupValue}_{$y}_{$plan}", "input_premi box"); ?></td>
                                                        <td><?php $this->DBForm->jpInput("plan_premiFemale_{$PayModeValue}_{$_GroupValue}_{$y}_{$plan}", "input_premi box"); ?></td>
                                                        <?php } else { ?>
