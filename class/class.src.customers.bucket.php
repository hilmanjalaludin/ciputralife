<?php
	...
	*/
	...
	/* function set style css **/
		...
		private function getUserList2()
		{
			if( $this->getSession('handling_type')==1 ){
				$sql = "Select a.* from tms_agent a inner join cc_agent b on a.id=b.userid  where a.user_state =1 and a.handling_type in('4') ";
			}
			else if( $this->getSession('handling_type')==2){
				$sql = "Select a.* from tms_agent a inner join cc_agent b on a.id=b.userid  where a.user_state =1 
						and a.handling_type in('4') 
						and a.mgr_id='".$this->getSession('UserId')."'";
			}
			else if( $this->getSession('handling_type')==3){
				$sql = "Select a.* from tms_agent a inner join cc_agent b on a.id=b.userid  where a.user_state =1 
		...
		{
			$sql = "select a.BranchCode, a.BranchName from t_lk_branch a";
			
			$qry = $this -> query($sql);
			foreach( $qry -> result_assoc() as $rows )
			{
				$datas[$rows['BranchName']] = $rows['BranchName'];
			}
			return $datas;	
		}
		/*
		private function getcity()
		{
			$sql = "select upper(a.CityNames) as CityNames from t_lk_city a";
			
			$qry = $this -> query($sql);
			foreach( $qry -> result_assoc() as $rows )
			{
				$datas[$rows['CityNames']] = $rows['CityNames'];
			}
			return $datas;	
		}
		*/
		...
		private function getCampaignAssigment($cond='')
		{
			...
			$sql = "select a.CampaignId, c.CampaignNumber, c.CampaignName from t_gn_customer a
		 ...
		 ?>
			...
			<div id="result_content_add" class="box-shadow" style="padding-bottom:4px;margin-top:2px;margin-bottom:8px;padding-top:4px;">
				<table cellpadding="8px;">
					<tr>
						<td class="text_header"> Customer CIF Number</td>
						<td><?php $this -> Form -> jpInput('cust_fine_code','input_text', $this->escPost('cust_fine_code'));?></td>
					...
					<tr>
						...
						<td class="text_header"> Agent </td>
						<td><?php $this -> Form -> jpCombo('user_id', 'select', $this -> getUserList2(),$this->escPost('user_id')) ?> </td>
					</tr>
					
					<tr>
						<td class="text_header"> Customer DOB <br> <font color=red>Format : "yyyy-mm-dd"<br> <font color=red>Or : "yyyy-mm"<br> <font color=red>Or : "yyyy"</td>
						<td><?php $this -> Form -> jpInput('cust_dob','input_text', $this->escPost('cust_dob'));?><br> <font color=blue>Exp : 2014-12-01 Or 2014-12 Or 2014</td>
						<td class="text_header"> Cust City </td>
						<td><?php $this -> Form -> jpCombo('city', 'select', $this -> getcity(),$this->escPost('city')) ?></td>
					</tr>
